...
*"CoT 감시 우려"를 느낌으로 매기면 논쟁이 안 끝나므로, 판별 기준 세 가지로 분해한다. (그 우려가 애초에 무엇을 걱정하는 것인지는 → Q5)*

| 판별 기준 | 질문 |
|---|---|
| ① **조절 가능성 (test-time controllability)** | 답할 때 루프 수를 늘릴 수 있나? |
| ② **대체 관계 (substitution)** | 루프를 늘리면 중간 풀이 텍스트가 줄어드나? |
| ③ **의도성 (intent)** | 그게 설계 목적인가, 부수 효과인가? |

| 방식 | ① 조절 | ② 대체 | ③ 의도 | 등급 |
|---|---|---|---|---|
| **고정 2회** (Nanbeige) | ❌ 상수로 박힘 | ❌ | ❌ | **없음** |
| **router 루프** (MoR) | △ 토큰별로 다름 | ❌ | ❌ 목적은 효율 | **낮음** |
| **가변 대량 루프** (Huginn) | ✅ 추론 시 최대 50회 | ✅ | ✅ **목적 자체가 그것** | **실체 있음** |

**고정 2회가 왜 무해한가**가 핵심이다. 루프 수가 코드에 `num_loops=2`로 박혀 있으면 모델은 어려운 문제가 나와도 **더 돌 수가 없다.** 그냥 44층짜리 모델과 똑같이 행동한다. 그런데 44층 모델이 22층 모델보다 더 위험하다고 하지는 않는다. 그래서 우려가 성립하지 않는다 — **원문이 옳은 부분.**

**Huginn이 왜 다른가**는 논문 제목이 답이다 — "Scaling up Test-Time Compute with **Latent Reasoning**". 텍스트를 더 뽑는 대신 **루프를 더 도는 것으로 추론을 확장하겠다**는 것이 대놓고 목표다. 즉 ①②③이 모두 걸린다. 부작용(side effect)이 아니라 기능(feature)이다.
...
### Q5. "CoT 감시 우려"는 정확히 무엇을 걱정하는 것인가?

*표의 마지막 칸이 재는 대상. 이게 무엇인지 모르면 등급 자체가 의미 없는 딱지가 된다.*

**CoT monitoring(chain-of-thought monitoring, 사고 사슬 감시)** 은 모델이 최종 답을 내기 전에 **텍스트로 뱉어 놓는 중간 풀이 과정을, 사람이나 자동 감시기가 읽어서 이 모델이 지금 무슨 의도인지 확인하는 것**이다.

이것이 중요한 이유는 AI 안전 쪽에서 **거의 유일하게 공짜로 얻은 창문(window)** 이기 때문이다. 모델 내부는 숫자 덩어리라 읽을 수 없는데, 중간 풀이만은 **사람 말로 적혀 있어서 그냥 읽으면 된다.** 실제로 모델이 "이건 들키면 안 되니 이렇게 하자" 같은 내용을 풀이에 그대로 적어 놓는 사례가 관측돼 왔고, 이상 행동 조사에 실제로 쓰인다 (Fortune 기사에서 Peter Wildeford가 조사에 CoT 분석이 결정적이었다고 든 것이 이 얘기).

**문제는 이 창문이 설계된 것이 아니라 우연히 생겼다는 점이다.** 모델이 자연어로 생각하도록 학습됐기 때문에 부수적으로 열린 창문이지, 누가 열어 둔 것이 아니다. 그래서 **구조가 바뀌면 그냥 닫힐 수 있다.**

걱정하는 시나리오는 이렇다:

```
지금:   문제 → "먼저 A를 계산하고… 그러면 B니까…" (텍스트로 다 보임) → 답
우려:   문제 → [루프 50바퀴, 전부 내부 숫자로만 진행]        → 답
                 ↑ 여기서 실제 추론이 다 일어나는데 아무것도 안 보임
```

이 상태의 내부 표현을 부르는 별명이 **neuralese(뉴럴리즈)** — "기계는 읽지만 사람은 못 읽는 언어". Steven Adler가 "업계에 몇 안 되는 redline(레드라인) 중 하나를 넘는 것"이라고 한 것이 이 지점이다.

**그래서 Astra는 어느 칸인가?** → **공개 정보가 없다.** 이것이 논쟁이 끝나지 않는 이유다. 원문은 첫 번째 칸일 것으로 보고 김을 뺐고, 안전 진영은 세 번째 칸일 가능성을 걱정한 것이다. OpenAI의 Jakub Pachocki는 "CoT 모니터링을 중시한다"고만 하고 구조는 추후 공개하겠다고 했다.
