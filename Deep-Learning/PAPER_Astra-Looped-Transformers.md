...
3. **"이 기법이 CoT를 가린다"는 보도는 자동으로 따라 나오지 않는다.** 레이어 재사용은 보통 레이어처럼 다음 토큰을 뽑기 전 hidden state에서 계산을 더 하는 것뿐. 굳이 해석하면 "루프를 많이 돌면 중간 추론 토큰을 덜 뽑아도 되니 계산이 latent로 숨는다"인데, 그건 **모델을 키워도 똑같이 생기는 현상**이다.
