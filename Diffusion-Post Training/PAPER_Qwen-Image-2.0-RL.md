...
### Q5. "기존 RL은 LoRA에서만 검증됐다"는 논문 주장은 맞나?

**"오직 LoRA"가 아니라 "주로(primarily) LoRA에서 검증됐다"** 가 원문이다. 그리고 이 주장은 **대상을 정확히 봐야** 한다.

- 논문이 인용한 건 Z-Image 같은 **완성 모델**이 아니라 **RL 기법을 제안한 방법론 논문**(Flow-GRPO, DiffusionNFT, GRPO-Guard)이고, "그 기법들이 실험을 주로 LoRA로 검증했다"는 말이다.
- **반례가 있다**: [Z-Image](PAPER_Z-Image.md)는 6B **풀 모델(full-parameter)** 을 2단계 RLHF(1단계 offline DPO=텍스트·개수 같은 객관 기준, 2단계 online GRPO=미학·포토리얼리즘)로 정렬하고 **가중치까지 공개**했다. 즉 "풀파라미터 RLHF 자체가 없었다"는 식으로 읽으면 사실과 안 맞는다. FLUX.2/klein은 후처리 방식이 미공개라 불명.

→ 그래서 이 "LoRA뿐" 서술은 **방법론 연구에 국한하면 대체로 맞지만, 완성 오픈 모델까지 넣으면 과장**이다(자기 기여를 부각하는 프레이밍). 반례를 인정하고 나면 이 논문에 남는 진짜 새로움은 (a) 방법론을 **문서로 풀어쓴 재현성**(단, 코드·데이터는 이 논문도 미공개 — Z-Image도 학습코드·reward·데이터 비공개라 절반짜리 우위), (b) **T2I+편집 여러 작업 × 여러 보상 × OPD 병합**을 한 파이프라인에 묶은 구성이다.

*참고 — DPO vs online RL 구분*: DPO는 보상 모델·rollout 없이 미리 만든 chosen/rejected 쌍으로 직접 최적화(offline). GRPO(이 논문 중심)는 보상 모델을 두고 rollout로 그룹을 뽑아 online 최적화. 자세한 비교는 Q7. Z-Image는 DPO+GRPO 둘 다, Qwen-Image-2.0-RL은 GRPO online 중심 + OPD.

### Q6. pointwise vs pairwise가 정확히 뭘 의미하나? (개념)

*4.1이 "왜 pointwise가 이기는가"(실험·RL 연결)를 다뤘다면, 여기선 개념 자체를 초보용으로 푼다.*

한 문장으로: **pairwise = "등수", pointwise = "점수".**

| | pairwise(쌍대비) | pointwise(점수식) |
|---|---|---|
| 입력 | 이미지 **2장** | 이미지 **1장** |
| 사람에게 묻는 것 | "어느 쪽이 더 나아?" | "5점 만점에 몇 점?" |
| 배우는 것 | **순위**(상대 우열) | **절대 점수**(척도 위 위치) |
| 정보 성격 | 서수(ordinal) — 방향만 | 기수(cardinal) — 방향 + **크기(간격)** |

**핵심 차이 — 예시**: 이미지 A·B·C를 채점하면,
- pairwise는 "A > B > C"(1·2·3등)만 안다. A와 B 사이 간격도 B와 C 사이만큼 벌어진 것처럼 오해할 수 있다.
- pointwise는 "A=4.5, B=4.4, C=2.0"을 안다. 즉 **"A·B는 거의 같고 C만 확 나쁘다"** 는 간격까지 담는다.

**RL에 왜 결정적인가**: GRPO의 advantage는 "그룹 평균보다 얼마나 좋은가"라는 **크기**가 필요하다. pointwise면 (4.5−평균)처럼 크기 조절된 신호가 나오지만, pairwise는 방향만 알고 크기가 흐릿하다. 게다가 고품질 모델끼리 pairwise 비교는 "질감 미세차"로 쪼그라들어 다른 축(구조·정렬)을 못 배우는 편향도 있다. 그래서 이 논문은 pointwise를 택했다.

> ⚠️ 헷갈리기 쉬운 점: **pairwise(4.1) ≠ DPO(Q7)**. pairwise는 *보상 모델을 학습할 때* 쓰는 라벨 형태이고, DPO는 *정책 모델을 정렬할 때* 쓰는 방법이다. 둘 다 "쌍"을 쓰지만 층위가 다르다. 이 논문은 pairwise 보상도, DPO도 안 쓴다(= pointwise 보상 + GRPO).

### Q7. DPO vs GRPO는 뭐가 다른가? (RLHF 방법론)

*왜 여기 두나: 이 논문은 GRPO를 쓰고 Z-Image는 DPO+GRPO를 쓴다. 둘의 차이를 알아야 "왜 이 논문이 GRPO 중심인가"가 읽힌다.*

한 문장으로: **DPO = 고정된 정답/오답 쌍으로 조용히 흉내내기, GRPO = 매번 새로 시도하고 채점받으며 탐색하기.**

| | DPO (Direct Preference Optimization) | GRPO (Group Relative Policy Optimization) |
|---|---|---|
| 보상 모델(채점관) | **없음** — 선호 쌍으로 직접 | **필요** — 따로 학습 |
| 데이터 | 미리 만든 chosen/rejected 쌍(고정) | 학습 중 모델이 직접 생성(rollout) |
| off/on-policy | **off-policy**(남이 만든 고정 데이터) | **on-policy**(지금 내 모델 생성물) |
| RL인가 | 엄밀히는 아님(분류 비슷한 손실) | 예(policy gradient) |
| 비용 / 안정성 | 싸고 안정적·단순 | 비싸고 불안정(보상 해킹 위험) |
| 한계 | 고정 데이터 밖은 개선 약함 | 채점관 구축 비용 + rollout 비쌈 |

**DPO의 핵심** = "보상 모델을 정책 안에 숨긴다." RLHF 최적해의 닫힌 형태를 거꾸로 뒤집어, 채점관 없이 선호 쌍만으로 정책을 직접 학습한다. 손실(평문):

> 손실 = −log σ( β·[ (logπ(정답) − logπref(정답)) − (logπ(오답) − logπref(오답)) ] )

즉 "현재 모델이 원본(πref)보다 정답엔 더 높은 확률, 오답엔 더 낮은 확률을 주도록" 미는 것. πref가 원본에서 멀어지지 않게 잡아준다.

**GRPO의 핵심** = "형제들과 비교해서 배운다." PPO에서 무거운 critic(가치함수)을 없애고, 같은 프롬프트로 여러 장(G)을 뽑아 **그룹 평균을 기준선**으로 삼는다(그래서 Group *Relative*). advantage = (내 점수 − 그룹 평균) / 표준편차. 채점관이 매번 새 생성물에 점수를 주니 **계속 새 영역을 탐색**한다. (수식 세부는 5.1~5.2.)

**왜 실무는 둘을 순서로 쓰나** — Z-Image가 예: ① DPO(offline)로 텍스트·개수 같은 **객관** 기준을 안정적으로 거칠게 정렬 → ② GRPO(online)로 미학·포토리얼리즘 같은 **주관** 품질을 탐색하며 미세조정. 반면 **이 논문은 GRPO 단독**이고, DPO 단계 없이 처음부터 online GRPO를 풀파라미터로 **안정화**하는 데 집중한다(하이브리드 CFG·timestep 샘플링). 그 대신 여러 GRPO 교사를 OPD로 병합.
...
- **"기존은 LoRA뿐" 프레이밍은 과장** — Z-Image가 풀 모델 RLHF(DPO+GRPO) 공개라는 반례 (Q5 참조). 방법론 연구에 국한할 때만 성립.
