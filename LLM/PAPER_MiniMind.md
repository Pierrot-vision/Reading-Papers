...
**① GQA** — 질의는 8갈래인데 키·값은 4갈래만 만들어, 캐시에 쌓이는 키·값을 절반으로 줄인다. → 왜 키·값만 줄이는지 **Q14**, 코드로 한 줄씩 **Q15**, 왜 처음부터 8개를 안 만드는지 **Q16**, 줄여도 손해가 작은지(실험) **Q17**.
...
- **블록의 제자리 덧셈이 어텐션 출력을 덮어씀**: `MiniMindBlock`의 `hidden_states += residual`이 어텐션 모듈이 돌려준 텐서를 **그 자리에서** 바꾼다. 그래서 `self_attn`에 forward hook을 걸어 출력을 복사 없이 저장하면, 나중에 보면 `어텐션 출력 + 잔차`로 바뀌어 있다(실측: 재계산 대조 오차가 잔차 크기처럼 층마다 0.16 → 3.7로 커짐, `.clone()`으로 저장하면 1e-6). 모델 동작엔 문제없고, 분석·해석용 훅을 쓸 때만 함정. → Q14
...
### Q14. 왜 질의는 8갈래인데 키·값은 4갈래인가?

**질문(Q)은 한 번 쓰고 버리지만, 키·값(K·V)은 계속 쌓아둬야 하기 때문**입니다. 드는 비용이 다르니 비싼 쪽만 줄인 것입니다.

#### 1. Q·K·V가 각각 무엇인가 — 도서관 비유

| | 역할 | 비유 |
|---|---|---|
| **Q** (질의) | 지금 토큰이 **찾고 싶은 것** | 내 질문지 |
| **K** (키) | 앞 토큰들이 **무엇을 가졌는지** 알리는 표시 | 책마다 붙은 색인 카드 |
| **V** (값) | 앞 토큰들의 **실제 내용** | 책 본문 |
| **헤드(갈래)** | 여러 관점으로 동시에 찾기 | 질문지를 여러 장 들고 가기 |

#### 2. 왜 K·V만 줄이나 — 비용이 비대칭이다

문장을 한 토큰씩 만들 때를 생각하면 됩니다.
- 새 토큰의 **Q**는 "지금 무엇을 볼까"를 계산하는 데 **한 번 쓰고 끝**
- 새 토큰의 **K·V**는 **이후 모든 토큰이 다시 들여다봐야** 하니 계속 보관 → 이게 KV 캐시

실제 모델로 확인하면, 캐시에는 K·V만 있고 Q는 없습니다.

```
한 층의 K 캐시 모양: (1, 15, 4, 96) = [배치, 토큰수, kv헤드 4, 헤드차원 96]
```

그래서 **Q 헤드 수는 메모리에 영향이 없고, K·V 헤드 수는 캐시 크기에 그대로 곱해집니다.**

| 방식 | kv 헤드 | 토큰 1개당 캐시 (fp16, 8층) | 32,768토큰을 채우면 |
|---|---|---|---|
| MHA (공유 없음) | 8 | 24,576 B | **768 MiB** |
| **GQA (MiniMind)** | **4** | **12,288 B** | **384 MiB** |
| MQA (전부 공유) | 1 | 3,072 B | 96 MiB |

모델 가중치 자체는 fp16으로 약 128MB입니다. 그런데 공유 없이 문맥을 끝까지 채우면 **캐시가 모델의 6배**가 됩니다. 긴 문장이나 큰 배치에서는 가중치보다 캐시가 메모리를 먼저 잡아먹는 거죠. 덤으로 `k_proj`·`v_proj`가 768→384로 작아져 파라미터도 **4.72M(7.4%)** 줄어듭니다.

#### 3. 그럼 질문(Q)도 4개로 줄이면?

줄일 이유가 없습니다. Q는 메모리 부담이 없는데, 줄이면 **보는 관점의 수**가 줄어 표현력만 떨어집니다.

반대로 K·V는 **여러 질문이 같은 색인을 함께 봐도 손해가 작다**는 게 실험으로 알려져 있습니다(GQA 논문, Ainslie et al. 2023). 전부 1개로 합치면(MQA) 품질이 눈에 띄게 떨어지지만, 몇 그룹만 남기면 원래 방식(MHA)의 품질에 거의 붙고 속도는 MQA에 가깝습니다. (직접 돌린 작은 실험은 Q17)

#### 4. 실측: K·V를 같이 쓰는 헤드는 같은 곳만 보나? — 아니다

학습된 MiniMind에 문장 `北京是中国的首都，猫和狗都是常见的宠物。`를 넣고, 헤드마다 어디를 보는지 비교했습니다. `repeat_kv` 순서상 질의 헤드 (0,1) (2,3) (4,5) (6,7)이 각각 같은 K·V를 씁니다. (직접 계산한 어텐션 출력이 실제 모듈 출력과 오차 1e-6 이내로 일치하는 것까지 확인했습니다.)

| 층 | 같은 K·V 쌍의 유사도 | 다른 K·V 쌍의 유사도 |
|---|---|---|
| 0 | 0.893 | 0.876 |
| 1 | 0.896 | **0.911** ← 오히려 다른 쌍이 더 비슷 |
| 2 | 0.918 | 0.849 |
| 3 | **0.916** | **0.739** |
| 4 | 0.800 | 0.732 |
| 5 | 0.810 | 0.739 |
...
(1.0 = 두 헤드가 완전히 같은 곳을 봄)

같은 K·V를 쓰면 대체로 더 비슷하지만 **1.0은 아닙니다.** 1층처럼 공유 여부가 별 차이를 안 내는 층도 있습니다. 참고로 이 작은 모델은 다른 그룹끼리도 0.73~0.91로, 헤드들이 원래 꽤 비슷한 곳을 봅니다.

2층에서 `狗`(개) 토큰이 헤드별로 가장 많이 보는 곳:

```
헤드 0 (그룹 0): '和' 0.49  '狗' 0.30  '，' 0.09
헤드 1 (그룹 0): '和' 0.52  '，' 0.18  '狗' 0.13
헤드 2 (그룹 1): '狗' 0.62  '和' 0.13  '猫' 0.12
헤드 3 (그룹 1): '狗' 0.45  '猫' 0.22  '，' 0.16
헤드 4 (그룹 2): '猫' 0.31  '狗' 0.29  '和' 0.19
헤드 5 (그룹 2): '猫' 0.31  '狗' 0.27  '和' 0.12
헤드 6 (그룹 3): '和' 0.43  '，' 0.23  '狗' 0.16
헤드 7 (그룹 3): '，' 0.39  '和' 0.28  '狗' 0.14   ← 같은 그룹인데 1순위가 다름
```

**색인(K·V)이 같아도 질문(Q)이 다르면 다른 곳을 봅니다.** 헤드 6과 7은 똑같은 K·V를 쓰는데 하나는 `和`, 하나는 `，`를 가장 많이 봅니다. 그래서 K·V를 절반으로 줄여도 "8가지 관점"은 상당 부분 살아 있습니다.

#### 5. 왜 하필 4인가 — 설계 선택

조건은 하나입니다. **8이 kv 헤드 수로 나누어떨어져야** 복제 횟수가 정수가 됩니다(8 ÷ 4 = 2). 그래서 고를 수 있는 값은 1, 2, 4, 8뿐입니다.

| 모델 | q : kv | 공유 비율 |
|---|---|---|
| **MiniMind-3** | 8 : 4 | 2:1 |
| Qwen3-0.6B / 1.7B | 16 : 8 | 2:1 |
| Qwen3-4B / 8B | 32 : 8 | 4:1 |
| Qwen3-30B-A3B | 32 : 4 | 8:1 |

큰 모델일수록 더 많이 공유합니다. 캐시 부담이 크고, 헤드가 많아 줄여도 여유가 있기 때문입니다. MiniMind는 가장 작은 Qwen3와 같은 2:1을 골랐습니다.

다만 **4가 최선이라는 실험은 저장소에 없습니다.** 이전 버전 minimind2는 8:2(4:1)였는데 minimind-3에서 8:4로 바꿨고, 바꾼 이유도 문서에 적혀 있지 않습니다.

### Q15. "8개 만들 것을 4개만 만들고 2번씩 복사한다" — 코드에서는 한 줄씩 어떻게 되나?

#### 한눈에

네, 정확합니다. **질의는 8개 다 만들고, 키·값은 4개만 만든 뒤 2번씩 복사해서 8개로 맞춥니다.**

```
질의  q_proj  768 → 768  = 96 × 8   →  Q0  Q1  Q2  Q3  Q4  Q5  Q6  Q7
키    k_proj  768 → 384  = 96 × 4   →  K0      K1      K2      K3
                                        │ 복사  │ 복사  │ 복사  │ 복사
repeat_kv 후                         →  K0  K0  K1  K1  K2  K2  K3  K3
                                        ↕   ↕   ↕   ↕   ↕   ↕   ↕   ↕
짝지어 어텐션                            Q0  Q1  Q2  Q3  Q4  Q5  Q6  Q7
```

실제 텐서로 확인하면 칸 0과 1이 숫자까지 똑같습니다.

```
칸 0 <- 원본 키 0: [ 0.514, -0.291, -0.093 ...]
칸 1 <- 원본 키 0: [ 0.514, -0.291, -0.093 ...]   ← 같음
칸 2 <- 원본 키 1: [-0.512, -0.197, -1.119 ...]
칸 3 <- 원본 키 1: [-0.512, -0.197, -1.119 ...]   ← 같음
```

`칸0 == 칸1` → True, `칸0 == 칸2` → False. 값(V)도 똑같이 복사됩니다.

핵심은 **복사는 저장하지 않는다**는 점입니다(자세히 → 아래 4-5).
- 캐시에 넣는 건 복사 **전**의 4개뿐 → 메모리 절반
- 복사는 계산하는 순간에만 잠깐 만들어서 쓰고 버림

그래서 Q0과 Q1은 **똑같은 키를 보지만 질문이 달라서** 서로 다른 곳에 주목할 수 있습니다(Q14의 `狗` 예시에서 같은 그룹인 헤드 6·7이 1순위로 보는 토큰이 달랐던 것).

#### 코드 한 줄씩

코드는 저장소 원문 그대로이고, 주석의 모양과 숫자는 모두 **학습된 가중치로 0층을 실제 실행해서 찍은 값**입니다.

- 예시 입력: `北京是中国的首都` → 토큰 `['北京', '是中国', '的', '首']` (4토큰), 다음 단계에서 `'都'` 1토큰 추가
- 검증: 직접 따라 계산한 결과가 실제 모듈 출력과 **처음 입력 단계 1.5e-7, 추가 생성 단계 0.0**으로 일치, 캐시도 동일

##### 0. RoPE 각도표 미리 만들기 — `precompute_freqs_cis` (L62–78)

```python
def precompute_freqs_cis(dim: int, end: int = int(32 * 1024), rope_base: float = 1e6, rope_scaling: dict = None):
    # dim=96(헤드 차원), end=32768(최대 위치)
    freqs, attn_factor = 1.0 / (rope_base ** (torch.arange(0, dim, 2)[: (dim // 2)].float() / dim)), 1.0
    #   → 회전 속도 48종: 1.000 rad/토큰(가장 빠름) … 1.33e-06 rad/토큰(가장 느림)
    if rope_scaling is not None:
        ...                                        # L64-73 YaRN 외삽. 기본값 None이라 건너뜀
    t = torch.arange(end, device=freqs.device)     # 위치 0, 1, 2, …, 32767
    freqs = torch.outer(t, freqs).float()          # (32768, 48)  위치 × 속도 = 회전각
    freqs_cos = torch.cat([torch.cos(freqs), torch.cos(freqs)], dim=-1) * attn_factor   # (32768, 96)
    freqs_sin = torch.cat([torch.sin(freqs), torch.sin(freqs)], dim=-1) * attn_factor   # (32768, 96)
    return freqs_cos, freqs_sin
```

- 96칸을 **48쌍**으로 묶어, 쌍마다 다른 속도로 돌립니다. 빠른 쌍은 가까운 위치 차이, 느린 쌍은 먼 위치 차이를 구별합니다.
- 위치 3이면 가장 빠른 쌍은 **3 rad(약 172도)**, 가장 느린 쌍은 **0.000004 rad**만 돕니다.
- `cat`으로 같은 각도를 두 번 붙여 96칸을 만듭니다. 실측으로 위치 3의 앞 3칸 `[-0.990, -0.628, -0.116]`과 48~50칸이 똑같습니다. 즉 **i번째 칸과 i+48번째 칸이 한 쌍**입니다.
- 학습 파라미터가 없고 모델 생성 때 한 번 계산해서 버퍼로 둡니다.

##### 1. 회전시키기 — `apply_rotary_pos_emb` (L80–84)
...
- 한 쌍(앞 칸 a, 뒤 칸 b)에 대해 풀면 `새 a = a·cos − b·sin`, `새 b = b·cos + a·sin` — 평면에서 각도만큼 **돌리는** 식입니다.
- `cos`는 (토큰수, 96)인데 `unsqueeze(1)`로 (토큰수, 1, 96)이 되어, (배치, 토큰수, **헤드**, 96) 모양의 q·k에 모든 헤드에 똑같이 적용됩니다.
- 질의 8개, 키 4개 — 헤드 수가 달라도 같은 함수로 처리됩니다.

##### 2. 4개를 8칸으로 복사 — `repeat_kv` (L86–89)

```python
def repeat_kv(x: torch.Tensor, n_rep: int) -> torch.Tensor:
    bs, slen, num_key_value_heads, head_dim = x.shape                     # (1, 4, 4, 96)
    if n_rep == 1: return x                                               # kv=8이면 복사 안 함
    return (x[:, :, :, None, :]                                           # (1, 4, 4, 1, 96)
             .expand(bs, slen, num_key_value_heads, n_rep, head_dim)      # (1, 4, 4, 2, 96)
             .reshape(bs, slen, num_key_value_heads * n_rep, head_dim))   # (1, 4, 8, 96)
```

| 단계 | 모양 | 실제로 복사하나? |
|---|---|---|
| `[:, :, :, None, :]` | (1, 4, 4, **1**, 96) | 아니오 — 크기 1 축만 끼움 |
| `.expand(.., 2, ..)` | (1, 4, 4, **2**, 96) | **아니오** — 같은 메모리를 두 번 가리킴 (그 축 stride = 0) |
| `.reshape(.., 8, ..)` | (1, 4, **8**, 96) | **예** — 여기서 처음 실제 복사 |

크기 1 축을 **키 헤드 축 바로 뒤**에 끼웠기 때문에 펼친 순서가 이렇게 됩니다.

```
실제 코드  [:, :, :, None, :]  →  K0 K0 K1 K1 K2 K2 K3 K3   (확인: 칸1 = K0, 칸2 = K1)
만약      [:, :, None, :, :]  →  K0 K1 K2 K3 K0 K1 K2 K3   (짝이 (0,4)(1,5)… 로 바뀜)
```

그래서 질의 헤드 (0,1) (2,3) (4,5) (6,7)이 각각 같은 키·값을 씁니다. 어느 순서든 계산은 성립하지만, Qwen3 가중치와 짝이 맞아야 하므로 순서가 정해져 있습니다.

##### 3. 부품 준비 — `Attention.__init__` (L91–109)

```python
class Attention(nn.Module):
    def __init__(self, config: MiniMindConfig):
        super().__init__()
        self.num_key_value_heads = config.num_attention_heads if config.num_key_value_heads is None else config.num_key_value_heads  # 4
        self.n_local_heads = config.num_attention_heads          # 8   ← 질의 갈래
        self.n_local_kv_heads = self.num_key_value_heads         # 4   ← 키·값 갈래
        self.n_rep = self.n_local_heads // self.n_local_kv_heads # 2   ← 복사 횟수 (8을 4로 나누어떨어져야 함)
        self.head_dim = config.head_dim                          # 96
        self.is_causal = True                                    # 미래 토큰을 못 보게
        self.q_proj = nn.Linear(config.hidden_size, config.num_attention_heads * self.head_dim, bias=False)  # 768 → 768
        self.k_proj = nn.Linear(config.hidden_size, self.num_key_value_heads * self.head_dim, bias=False)   # 768 → 384 ← 처음부터 절반
        ...
        self.q_norm = RMSNorm(self.head_dim, eps=config.rms_norm_eps)   # 헤드 하나(96칸) 단위 정규화
        self.k_norm = RMSNorm(self.head_dim, eps=config.rms_norm_eps)
        self.attn_dropout = nn.Dropout(config.dropout)           # p = 0 (기본 꺼짐)
        self.resid_dropout = nn.Dropout(config.dropout)          # p = 0
        self.dropout = config.dropout
        self.flash = hasattr(torch.nn.functional, 'scaled_dot_product_attention') and config.flash_attn  # True
```

"8개인데 4개"는 여기서 결정됩니다. **키·값은 처음부터 384칸(4갈래 분량)만 만드는 층**입니다.

##### 4-1. 투영하고 갈래로 쪼개기 (L112–116)

```python
def forward(self, x, position_embeddings, past_key_value=None, use_cache=False, attention_mask=None):
    bsz, seq_len, _ = x.shape                                            # x (1, 4, 768)
    xq, xk, xv = self.q_proj(x), self.k_proj(x), self.v_proj(x)          # (1, 4, 768) / (1, 4, 384) / (1, 4, 384)
    xq = xq.view(bsz, seq_len, self.n_local_heads, self.head_dim)        # (1, 4, 8, 96)  질의 8개
    xk = xk.view(bsz, seq_len, self.n_local_kv_heads, self.head_dim)     # (1, 4, 4, 96)  키 4개
    xv = xv.view(bsz, seq_len, self.n_local_kv_heads, self.head_dim)     # (1, 4, 4, 96)  값 4개
```

`view`는 새로 계산하지 않고 **긴 줄을 96칸씩 잘라 볼 뿐**입니다.

```
k_proj(x)[토큰0, 칸 96:99]  = [0.716, -0.005, 0.924]
xk[토큰0, 키헤드1, 칸 0:3]   = [0.716, -0.005, 0.924]   ← 같은 숫자: 96~191칸이 곧 헤드1
```

##### 4-2. 헤드마다 크기 맞추기 — QK-Norm (L117)

```python
    xq, xk = self.q_norm(xq), self.k_norm(xk)      # 모양 그대로, 헤드(96칸)마다 따로 정규화
```

```
토큰0 키헤드 4개의 크기(RMS)  정규화 전: [0.721, 0.744, 0.717, 0.645]   ← 제각각
                            정규화 후: [1.224, 1.220, 1.228, 1.218]   ← k_norm.weight 평균 1.224로 통일
```

어느 헤드든 같은 크기가 되어, 다음 단계의 내적 점수가 한 헤드에서만 튀는 일을 막습니다. (RMSNorm 자체는 Q12)

##### 4-3. 위치 정보 싣기 — RoPE (L118–119)

```python
    cos, sin = position_embeddings                  # 각각 (4, 96): 위치 0~3의 각도
    xq, xk = apply_rotary_pos_emb(xq, xk, cos, sin) # 모양 그대로
```

```
질의헤드0 길이, 토큰별  회전 전: [11.868, 11.958, 12.032, 11.997]
                      회전 후: [11.868, 11.958, 12.032, 11.997]   ← 길이 보존 (돌리기만 함)
위치0 벡터 그대로? True (각도 0)   위치3 벡터 바뀜? True
```

정규화로 맞춘 크기는 건드리지 않고 **방향만** 위치만큼 돌립니다. 그래서 순서가 QK-Norm → RoPE입니다.

##### 4-4. 캐시 이어붙이고 저장 ★ (L120–123)

```python
    if past_key_value is not None:                       # 처음 입력엔 캐시 없음 → 건너뜀
        xk = torch.cat([past_key_value[0], xk], dim=1)   # 추가 생성 때: 과거 키 뒤에 새 키 붙이기
        xv = torch.cat([past_key_value[1], xv], dim=1)
    past_kv = (xk, xv) if use_cache else None            # ★ 캐시 저장: (1, 4, 4, 96)  ← 복사 전 4갈래
```

##### 4-5. 4 → 8 복사 ★ (L124)

```python
    xq, xk, xv = (xq.transpose(1, 2),                                   # (1, 8, 4, 96)
                  repeat_kv(xk, self.n_rep).transpose(1, 2),            # (1, 4, 4, 96) → (1, 4, 8, 96) → (1, 8, 4, 96)
                  repeat_kv(xv, self.n_rep).transpose(1, 2))            # (1, 8, 4, 96)
```

**L123 → L124 순서가 핵심**입니다. 저장이 복사보다 한 줄 먼저라서 **캐시에는 4갈래만** 들어가고, 복사한 8칸은 이번 계산에만 쓰고 버립니다. "메모리 절반"이 코드에서는 이 줄 순서로 구현됩니다.

`transpose(1, 2)`는 (배치, **토큰**, **헤드**, 96) → (배치, **헤드**, **토큰**, 96)로 축을 바꿉니다. 다음 줄의 행렬 곱을 헤드 8개에 대해 한꺼번에 하기 위해서입니다.

##### 4-6. 빠른 길로 갈지 정하기 (L125–127)
...
| `seq_len > 1` | True | **False** |
| 캐시 없음 | True | **False** |
| 패딩 마스크 없음 | True | True |
| **결과** | **플래시(SDPA) 경로** | **수동 경로** |

플래시 경로는 아래 L128–131을 하나로 합친 빠른 계산이고 결과는 같습니다. 아래 숫자는 이해를 위해 수동 경로로 풀어 찍은 것입니다.

##### 4-7. 점수표 — 누가 누구를 얼마나 볼까 (L128)

```python
        scores = (xq @ xk.transpose(-2, -1)) / math.sqrt(self.head_dim)   # (1, 8, 4, 4)  헤드마다 4×4
```

헤드 0의 점수표 (행 = 보는 토큰, 열 = 보이는 토큰):

```
            北京    是中国    的      首
北京       9.430   4.613   2.017   3.061
是中国     4.306  10.645   2.952   2.641
的         5.135   7.446   7.818   5.295
首         5.725   6.097   5.132   9.975
```

√96(약 9.8)으로 나누지 않으면 점수가 10배 가까이 커져(가장 큰 값 약 104) 다음 softmax가 한 칸에 몰빵합니다.
...
```
더하는 마스크                     더한 뒤
[[0, -inf, -inf, -inf],          [[ 9.430,  -inf,  -inf,  -inf],
 [0,    0, -inf, -inf],           [ 4.306, 10.645,  -inf,  -inf],
 [0,    0,    0, -inf],           [ 5.135,  7.446, 7.818,  -inf],
 [0,    0,    0,    0]]           [ 5.725,  6.097, 5.132, 9.975]]
```

`-seq_len:`로 **마지막 seq_len개 열에만** 마스크를 더하는 이유는 캐시 때문입니다. 1토큰 추가 때는 점수표가 (1행 × 5열)인데, 앞 4열(과거)은 전부 보여야 하고 마지막 1열만 1×1 마스크 `[[0]]`를 받습니다. 즉 아무것도 가리지 않습니다.
...
배치 안에 길이가 다른 문장이 섞여 빈칸(패딩)이 있을 때만 씁니다. 마스크 0인 자리에 −10억을 더해 안 보이게 합니다. 이 예시에서는 쓰지 않습니다.

##### 4-10. 비율로 바꾸고 값 섞기 (L131)

```python
        output = self.attn_dropout(F.softmax(scores.float(), dim=-1).type_as(xq)) @ xv   # (1, 8, 4, 96)
```

softmax 후 헤드 0의 주목 비율:

```
            北京    是中国    的      首       행 합
北京       1.000   0.000   0.000   0.000    1.000   ← 첫 토큰은 자기밖에 못 봄
是中国     0.002   0.998   0.000   0.000    1.000
的         0.039   0.392   0.569   0.000    1.000   ← '的'는 앞 '是中国'도 39% 봄
首         0.014   0.020   0.008   0.959    1.000
```

`@ xv`는 **이 비율대로 값(V)을 섞는** 계산입니다. 헤드 0에서 `首`의 출력은 `0.014×V(北京) + 0.020×V(是中国) + 0.008×V(的) + 0.959×V(首)`이고, 손으로 더한 결과와 일치했습니다. `scores.float()`는 softmax를 32비트로 안전하게 계산하려는 것이고, `attn_dropout`은 p = 0이라 아무 일도 하지 않습니다.

##### 4-11. 8갈래 합치고 섞기 (L132–134)

```python
    output = output.transpose(1, 2).reshape(bsz, seq_len, -1)   # (1, 8, 4, 96) → (1, 4, 8, 96) → (1, 4, 768)
    output = self.resid_dropout(self.o_proj(output))            # (1, 4, 768)
    return output, past_kv                                       # 출력 + 4갈래 캐시
```

`reshape`는 8개 헤드 결과를 **옆으로 이어붙입니다.** 실측으로 `output[토큰3, 칸 96:192]`가 헤드 1의 결과와 똑같았습니다. 이어붙이기만 하면 헤드끼리 섞이지 않으므로, `o_proj`(768 → 768)가 8개 관점을 한데 섞어 최종 출력을 만듭니다.

##### 5. 한 토큰 더 만들 때 — 질의는 1개, 키·값은 전부

`'都'`를 캐시와 함께 넣으면 같은 코드가 이렇게 흘러갑니다.

| 줄 | 처음 입력 (4토큰) | 1토큰 추가 (`'都'`) |
|---|---|---|
| L114 질의 | (1, 4, 8, 96) | (1, **1**, 8, 96) ← 새 토큰 것만 |
| L115 키 | (1, 4, 4, 96) | (1, 1, 4, 96) |
| L121 과거와 이어붙이기 | 건너뜀 | (1, **5**, 4, 96) ← 과거 4 + 새 1 |
| L123 캐시 저장 | (1, 4, **4**, 96) | (1, 5, **4**, 96) |
| L124 복사 후 키 | (1, 8, 4, 96) | (1, 8, **5**, 96) |
| L125 경로 | 플래시 | 수동 (`seq_len = 1`) |
| L128 점수표 | (1, 8, 4, 4) | (1, 8, **1**, **5**) ← 질의 1개 × 키 5개 |
| L132 출력 | (1, 4, 768) | (1, 1, 768) |

질의는 **이번 토큰 1개만** 만들고 쓰고 버리는데, 키·값은 **지나온 토큰 전부**를 계속 들고 다닙니다. 그래서 줄일 가치가 있는 건 키·값뿐이고, 캐시에 4갈래만 두고 계산 순간에만 8칸으로 복사하는 것입니다.

##### 한 장 요약 — 모양 흐름

```
x (1,4,768)
 ├ q_proj → (1,4,768)  → view (1,4,8,96) → q_norm → RoPE ─────────────── transpose → Q (1,8,4,96) ─┐
 ├ k_proj → (1,4,384)  → view (1,4,4,96) → k_norm → RoPE → [캐시 저장 4갈래] → repeat_kv (1,4,8,96) → K (1,8,4,96) ─┤
 └ v_proj → (1,4,384)  → view (1,4,4,96) ─────────────── → [캐시 저장 4갈래] → repeat_kv (1,4,8,96) → V (1,8,4,96) ─┤
                                                                                                                │
   scores = Q·Kᵀ ÷ √96 (1,8,4,4) → + causal mask → softmax → × V (1,8,4,96) ◄──────────────────────────────────┘
   → transpose + reshape (1,4,768) → o_proj (1,4,768)
```

### Q16. 어차피 복사해서 8개로 채울 거면, 왜 처음부터 8개를 만들지 않나?

헷갈리는 지점은 아마 이것일 겁니다. **"어차피 2개씩 복사해서 8개로 채울 거면, 처음부터 8개를 만들면 되지 않나?"**

답은 **복사가 목적이 아니라, "보관을 4개만 하는 것"이 목적**이라는 데 있습니다. 복사는 계산할 때 짝을 맞추려는 임시 요령일 뿐이고, 사실 복사 없이도 똑같이 계산할 수 있습니다.

#### 1. "처음부터 8개 만들기" vs "4개 만들고 복사" — 차이는 보관에 있다

| | 계산할 때 쓰는 키·값 | **캐시에 보관하는 키·값** |
|---|---|---|
| 처음부터 8개 만들기 | 8갈래 (모두 다른 내용) | **8갈래** |
| 4개 만들고 복사 (MiniMind) | 8칸 (2칸씩 같은 내용) | **4갈래** — 복사본은 계산 끝나면 버림 |

계산하는 순간만 보면 둘 다 8칸이라 복사가 쓸데없어 보입니다. 차이는 **계산이 끝난 뒤 무엇을 남겨두느냐**에 있습니다.

#### 2. 왜 보관이 문제인가 — 문장을 만들수록 쌓인다

모델은 글자를 한 토큰씩 만듭니다. 새 토큰을 만들 때마다:
- **질의**: "지금 무엇을 볼까"를 계산하는 데 한 번 쓰고 **버림**
- **키·값**: 다음 토큰들이 계속 다시 봐야 하므로 **쌓아둠**

```
토큰 생성 수   키·값 8갈래 보관    키·값 4갈래 보관    질의 보관
      1          0.02 MiB           0.01 MiB          0
    100          2.34 MiB           1.17 MiB          0
  1,000         23.44 MiB          11.72 MiB          0
 32,768        768.00 MiB         384.00 MiB          0
```

질의는 아무리 많아도 남는 게 없고, 키·값은 문장이 길어질수록 계속 불어납니다. 모델 가중치가 약 128MB인데 8갈래로 끝까지 채우면 캐시가 그 6배입니다. **줄여야 할 건 쌓이는 쪽(키·값)** 이고, 질의는 줄여봐야 아낄 게 없습니다.

#### 3. 비유 — 도서관

학생 8명(질의 8개)이 **각자 다른 질문**을 들고 자료를 찾습니다.

- 원래 방식: 학생마다 색인 카드 세트(키·값)를 하나씩, **8세트를 만들어 계속 보관**
- MiniMind 방식: 색인 카드는 **4세트만 보관**하고, **2명씩 한 세트를 같이 봄**

같은 카드 세트를 봐도 **질문이 다르니 찾아내는 책이 다릅니다.** Q14에서 같은 키·값을 쓰는 헤드 6과 7이 서로 다른 토큰(`和` vs `，`)을 가장 많이 봤던 게 이것입니다. 그래서 보관을 절반으로 줄여도 "8가지 관점"은 대부분 살아 있습니다.

#### 4. 그럼 "복사"는 왜 하나 — 곱셈 모양을 맞추려는 요령일 뿐

질의 8개와 키 4개를 그대로 곱하면 에러가 납니다.

```
Q (1, 8, 4, 96) @ K (1, 4, 4, 96)
에러: The size of tensor a (8) must match the size of tensor b (4) at non-singleton dimension 1
```

그래서 코드는 키를 2번씩 복사해서 8칸으로 맞춥니다. 그런데 **복사 없이도 똑같이 계산할 수 있습니다.** 질의 8개를 2명씩 4묶음으로 세워 한 세트를 같이 보게 하면 됩니다.

```
[방법 1] 코드 방식: 키를 복사해서 8칸     K (1, 8, 4, 96)
[방법 2] 복사 없음: 질의를 4묶음×2명      Q (1, 4, 2, 4, 96)   K (1, 4, 1, 4, 96)

두 방법 결과 최대 차이: 1.3e-06   ← 같음
```

즉 **복사는 본질이 아닙니다.** 본질은 **"질문 2개가 키·값 1세트를 함께 쓴다(공유한다)"** 이고, 복사는 파이토치 곱셈 모양을 맞추기 쉬운 구현 방법 중 하나일 뿐입니다.

#### 정리

1. 키·값은 문장이 길어질수록 **계속 쌓아둬야** 하는데, 질의는 **쓰고 버린다**
2. 그래서 쌓이는 키·값만 **4세트로 줄여 보관**하고, 질의 8개가 **2개씩 나눠 쓴다**
3. 코드상 곱셈 짝을 맞추려고 계산 순간에만 **2번씩 복사**할 뿐, 복사본은 보관하지 않는다

### Q17. 키·값을 8개에서 4개로 줄였다면, 나머지 4개는 불필요했던 건가?

**불필요했던 게 아니라, "조금 손해 보고 캐시를 절반 아끼는 거래"** 입니다. 8개일 때 키·값 8세트는 서로 꽤 겹치긴 하지만 완전히 같지는 않아서, 4세트로 줄이면 표현력이 **약간은** 줄어듭니다.

#### 실험: 키·값 헤드 수만 바꿔 처음부터 학습

폭 256·4층·질의 헤드 8개인 작은 모델을 저장소 README(10.9만 토큰)로 학습시켰습니다. 키·값 헤드 수 말고는 전부 같고, 설정마다 시드 2개로 돌렸습니다(1,500스텝, 100스텝마다 검증해 최저값). 손실은 낮을수록 좋습니다.

| 키·값 헤드 | 검증 손실 (평균) | kv=8 대비 | 두 시드 간 차이 | 캐시 / 토큰 |
|---|---|---|---|---|
| **8** (공유 없음) | 2.179 | — | 0.013 | 4,096 B |
| **4** (MiniMind 방식) | 2.196 | **+0.017** | 0.017 | 2,048 B |
| **2** | 2.239 | +0.060 | **0.076** ← 한 시드만 크게 나쁨 | 1,024 B |
| **1** (전부 공유) | 2.224 | **+0.045** | 0.011 | 512 B |

읽는 법:
- **8 → 4**: 손실이 +0.017 늘었지만, **같은 설정을 시드만 바꿔도 이 정도(0.013~0.017)는 흔들립니다.** 이 규모에서는 손해가 잡음과 구분이 안 될 만큼 작습니다.
- **8 → 1**: 두 시드 모두 확실히 나빠졌습니다(+0.045). 너무 줄이면 손해가 드러납니다.
- **kv=2**: 한 시드가 튀어서 1보다 나빠 보이지만, 시드 간 차이가 0.076이라 2와 1의 순서는 이 실험으로 판단할 수 없습니다.
- 키·값 층이 작아지면 파라미터도 조금 줄어서(kv=4에서 약 5%) 그 효과도 섞여 있습니다.

즉 **줄일수록 손해가 커지는 방향은 분명하지만, 절반(4개)까지는 손해가 매우 작다**는 게 결과입니다.

#### 왜 절반은 손해가 작나 — 8세트가 서로 많이 겹친다

도서관 비유로 이어가면, 학생 8명이 각자 색인 카드 세트를 만들어도 **8세트 중 상당수가 비슷한 내용**입니다. "이 토큰은 명사다", "이 토큰은 앞 문장과 관련 있다" 같은 정보는 여러 세트에 겹쳐 적힙니다.

그래서:
- **4세트로 줄여도** 겹치던 내용이 합쳐질 뿐, 찾을 수 있는 책은 대부분 그대로입니다. 게다가 **처음부터 4세트로 학습하면** 모델이 필요한 정보를 4세트 안에 나눠 담는 법을 스스로 배웁니다.
- **1세트까지 줄이면** 서로 다른 정보를 담을 칸이 모자라 못 찾는 책이 생깁니다. 이게 kv=1의 손해입니다.

"불필요"였다면 1세트까지 줄여도 손해가 없어야 하는데, 실제로는 있습니다. **절반쯤은 겹쳐서 줄여도 괜찮았고, 전부 없애도 될 만큼 쓸모없지는 않았다**가 정확한 표현입니다.

#### 그래서 몇 개로 할지는 거래로 정한다

| 선택 | 얻는 것 | 잃는 것 |
|---|---|---|
| 8 (공유 없음) | 가장 좋은 품질 | 캐시 최대 |
| **4 (MiniMind)** | 캐시 **절반** | 품질 손해 **아주 작음** |
| 1 (전부 공유) | 캐시 1/8 | 품질 손해가 **눈에 띔** |

큰 모델일수록 더 많이 공유합니다(Qwen3-8B는 32:8, Qwen3-30B-A3B는 32:4). 헤드가 많을수록 겹치는 부분도 많고, 캐시가 차지하는 메모리 부담도 크기 때문입니다.

**주의**: 이건 10만 토큰·500만 파라미터짜리 작은 실험이라 정확한 크기보다 **방향**만 믿는 게 맞습니다. 대규모 모델에서도 같은 방향(몇 그룹만 남기면 거의 손해 없음, 1개까지 줄이면 품질 저하)이 GQA 논문(Ainslie et al., 2023)에서 보고됐습니다.
...
```python
# 9) KV 캐시 모양 — Q14
out = m(ids, use_cache=True); out.past_key_values[0][0].shape   # (1, 15, 4, 96): 키 4갈래만 저장

# 10) 같은 K·V를 쓰는 헤드끼리 주목 패턴 유사도 — Q14
#     self_attn forward hook에서 출력은 반드시 .clone()으로 저장 (블록의 += residual이 덮어씀)
F.cosine_similarity(A[j, 1:], A[j+1, 1:], dim=-1).mean()       # 공유 쌍 (0,1)(2,3)… 0.80~0.92

# 11) repeat_kv 복사 확인 — Q15
kr = repeat_kv(k, 2)                                              # (1, L, 4, 96) → (1, L, 8, 96)
torch.equal(kr[:, :, 0], kr[:, :, 1]), torch.equal(kr[:, :, 0], kr[:, :, 2])   # True, False

# 12) 복사 없이 묶어서 계산해도 같다 — Q16
Qg = Q.view(1, 4, 2, L, 96); Kg, Vg = K4.unsqueeze(2), V4.unsqueeze(2)
out2 = (softmax(Qg @ Kg.transpose(-2, -1) / 96**0.5 + mask) @ Vg).reshape(1, 8, L, 96)   # 코드 방식과 차이 1.3e-06

# 13) 키·값 헤드 수만 바꿔 학습 (폭 256, 4층, 질의 8, README 10.9만 토큰, 1500스텝, 시드 2) — Q17
MiniMindConfig(vocab_size=6400, hidden_size=256, num_hidden_layers=4, num_attention_heads=8,
               num_key_value_heads=kv, max_position_embeddings=512)   # kv ∈ {8, 4, 2, 1}
# 최저 검증 손실 평균: 8 → 2.179 / 4 → 2.196 / 2 → 2.239 / 1 → 2.224
```
