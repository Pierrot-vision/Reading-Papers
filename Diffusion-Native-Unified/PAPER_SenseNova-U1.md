...
- 시퀀스 개수로 바꾸면(평균 길이 2K~8K 가정) **한 step에 시퀀스 약 800~3,000개** — GPT-3(320만)·요즘 LLM(400만~1,600만 token/배치) 관행 안의 표준 규모. 실제로는 수백 장 GPU에 쪼개 처리한 뒤 gradient를 모아 한 번에 업데이트(global batch).
- **epoch(에포크)와 구분**: epoch = 전체 데이터를 한 바퀴. 사전학습은 데이터가 커서 step으로 세고, RL처럼 데이터가 작은 단계(프롬프트 48개)는 같은 세트를 여러 바퀴 돌아 epoch로 셈.
- ⚠️ 실제 GPU 수·시퀀스 길이·배치 구성은 논문 미공개 — token/step만 확정, 시퀀스 개수는 역산 추정.

### Q5. 총 3.29T 토큰이면 학습량이 너무 적은 거 아냐?
