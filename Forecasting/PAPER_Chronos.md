...
| **quantile(분위수)** | 분포를 대표하는 지점들. 0.1분위 = 하위 10% 지점. **Bolt는 0.1~0.9의 9개**, **Chronos-2는 0.01~0.99의 21개**를 직접 회귀한다(config 실측). |
...
| 출력 | 20회 샘플링 → 경로 20개 | **9분위** 1회 직접 회귀 | **21분위** × 패치 |
...
### 3.5 입력 파이프라인 상세 — 실측 config와 텐서 추적

*이 절을 두는 이유: 3세대의 차이가 가장 선명하게 드러나는 곳이 입력단이고, 논문·README에 안 적힌 실제 하이퍼파라미터는 config를 직접 열어야만 확인되기 때문.*

#### 실측 config (HuggingFace `config.json` 직접 확인)

| | `chronos-t5-base` | `chronos-bolt-base` | `chronos-2` |
|---|---|---|---|
| d_model / d_ff / heads | 768 / 3072 / 12 | 768 / 3072 / 12 | 768 / 3072 / 12 |
| 층 | enc 12 + dec 12 | enc 12 + dec 12 | **enc 12 (디코더 없음)** |
| context_length | **512** | **2048** | **8192** |
| input_patch_size | — | **16** | **16** |
| input_patch_stride | — | **16** (겹침 없음) | **16** (겹침 없음) |
| output_patch_size | — | — | **16** |
| prediction / max_output_patches | 64 | 64 | **64패치 = 1024스텝** |
| quantiles | — | **9개** (0.1~0.9) | **21개** (0.01, 0.05, 0.1 … 0.95, 0.99) |
| 기타 | n_tokens 4096, [-15,15], num_samples 20, top_k 50 | use_reg_token | use_arcsinh **true**, time_encoding_scale 8192, use_reg_token |

#### 세 세대가 같은 데이터를 어떻게 다르게 보는가

온도 10칸 `[20.1, 20.3, 20.6, 21.0, 21.5, 22.1, 22.6, 23.0, 23.3, 23.5]` 을 실제 코드에 넣어 돌린 결과다.

**Chronos-1 (mean scaling + 양자화)**

```
1) scale = mean(|x|) = 21.80          ← 평균을 빼지 않고 나누기만 함
2) x / scale        = [0.92202 ... 1.07798]
3) 표현 범위 [-15, +15]를 4093칸으로 → bin 간격 0.0073318
4) 토큰 번호 = [2175, 2176, 2178, 2180, 2184, 2187, 2190, 2193, 2195, 2196]
```

이 시계열은 **4093칸 중 21칸 폭만** 쓴다(어휘의 0.5%). 3.4도가 움직이는데 스케일링 후에는 0.156만 움직이기 때문이다. 복원 오차:

```
원본   : 20.1    20.3    20.6    21.0    21.5    ...
복원   : 20.1378 20.2977 20.6173 20.9370 21.5762 ...
최대 오차 0.076도 = 이 계열 변동폭(3.4도)의 2.24%
```

**정밀도 붕괴가 언제 터지는가** — 같은 데이터가 켈빈(293.1K ~ 296.5K)으로 기록됐다면:

```
scale = 294.8                          ← 평균이 14배 커짐
bin 간격 = 30 × 294.8 / 4092 = 2.16 K
데이터 변동폭 = 3.4 K
→ 전체 시계열이 bin 1~2개 안에 들어감 → 사실상 전부 같은 토큰 → 신호 소멸
```

**단위를 바꿨을 뿐인데 신호가 사라진다.** 논문 §5.7이 인정하며 제안하는 해법이 *"큰 스케일·작은 분산 계열은 추론 시 표준화로 전처리하라"* — 즉 **토크나이저를 우회하라**는 뜻이다.

**Bolt / Chronos-2 (표준화)**

```
loc = 21.80, scale = 1.2091
표준화 결과 = [-1.406, -1.2406, -0.9924, -0.6616, -0.2481, 0.2481, 0.6616, 0.9924, 1.2406, 1.406]
arcsinh 적용 = [-1.1414, -1.0417, -0.876, -0.621, -0.2456, 0.2456, 0.621, 0.876, 1.0417, 1.1414]
```

| | 스케일링 후 범위 | 사용 해상도 |
|---|---|---|
| Chronos-1 | `[0.922, 1.078]` | 4093칸 중 **21칸** (0.5%) |
| Bolt / Chronos-2 | `[-1.406, 1.406]` | float 전체 |
| Chronos-2 (+arcsinh) | `[-1.141, 1.141]` | float 전체, 꼬리 압축 |

**같은 데이터인데 동적 범위가 18배 넓어지고, 양자화가 없으니 해상도 손실 자체가 사라진다.**

#### 텐서 모양 추적

**Chronos-1** (context 512, pred 64, vocab 4096)

```
입력 (한 채널)           [16, 512]            B, 시간      ← 실숫값
  ↓ scale = mean(|x|) → bucketize → 정수
                         [16, 512]  int64     ← 토큰 번호 (3 ~ 4095)
  ↓ EOS 붙이기           [16, 513]
  ↓ nn.Embedding(4096, 768)         ★ 다른 두 세대와 결정적으로 다른 지점
                         [16, 513, 768]
  ↓ T5 인코더 12층 + 디코더 12층
  ↓ generate(do_sample=True, num_return_sequences=20, top_k=50)
                         [16, 20, 64]  int64  ← 20개 경로 × 64스텝, 전부 토큰 번호
  ↓ centers[token] × scale
출력                     [16, 20, 64]  float
```

**Chronos-Bolt** (context 2048, 패치 16, 9분위)

```
입력                     [16, 2048]           B, 시간
  ↓ 표준화                                     ★ Chronos-1과 갈리는 지점
  ↓ unfold(size=16, step=16)
       값                [16, 128, 16]
       마스크            [16, 128, 16]
  ↓ concat               [16, 128, 32]        ← 입력 채널이 2개
  ↓ ResidualBlock(32 → 3072 → 768)
                         [16, 128, 768]
  ↓ [REG] 토큰 1개       [16, 129, 768]
  ↓ T5 인코더 12층       [16, 129, 768]
  ↓ 디코더 1스텝 (start 토큰 하나)
                         [16,   1, 768]       ★ 여기서 시퀀스가 1로 붕괴
  ↓ ResidualBlock(768 → 3072 → 9×64=576)
출력                     [16,   9,  64]       B, 9분위, 64스텝   ← 한 방
```

**왜 마스크를 값과 나란히 넣나**: 결측을 0으로 채우면 모델이 "0이라는 관측값"과 "값이 없음"을 구분 못 한다. 마스크를 붙이면 `[0.0, 0.0]`(결측)과 `[0.0, 1.0]`(진짜 0)이 다른 벡터가 된다. 실데이터 사전학습에서는 결측이 필연이라 필수다. PatchTST는 결측 없는 벤치마크만 다뤄서 필요가 없었다.

**패딩을 왼쪽에 붙이는 이유**: `Patch.forward`가 `torch.concat((padding, x), dim=-1)`로 **앞에** 붙인다. 뒤에 붙이면 가장 최근 관측이 패치 경계에서 밀려나는데, 예측에서 제일 중요한 건 마지막 칸이므로 앞을 희생한다.

**Chronos-2** (context 8192, 패치 16, 21분위, 변량 6개·예측 48스텝 과제)

입력 패치가 **48차원**이다 — `ResidualBlock(in_dim = input_patch_size * 3)`, 코드 주석 *"x3 for [time_embedding, patch, patch_mask]"*.

```
Bolt      : [ 값 16 | 마스크 16 ]                 = 32차원
Chronos-2 : [ 시간 16 | 값 16 | 마스크 16 ]        = 48차원
```

시간 인코딩은 **끝에서부터 센 상대 위치를 컨텍스트 길이로 나눈 값**이다.

```python
context_time_enc = torch.arange(start=-final_context_length, end=0, ...).div(time_encoding_scale)  # 8192
# 컨텍스트: [-8192, ..., -1] / 8192 = [-1.0, ..., -0.000122]
# 미래   : [0, ..., h-1] / 8192
```

**왜 필요한가**: RoPE는 패치 *사이*의 상대 위치를 주지만 패치 *안의* 16칸이 각각 몇 번째인지는 모른다. 시간 인코딩을 값과 나란히 넣으면 패치 내부 순서와 "예측 시점까지 얼마나 남았는가"가 동시에 들어가고, **미래 패치도 같은 임베딩 층을 태울 수 있게** 된다 — 미래는 값이 없고 시간만 있으니까.

```
context      [6, 8192]              (batch=변량 6개), 시간
group_ids    [6]                    = [0,0,1,1,1,2]
future_cov   [6, 48]                NaN 또는 값
  ...
  ↓ unfold(16, 16)
      ...
      마스크      [6, 512, 16]
  ↓ concat        [6, 512, 48]
  ↓ ResidualBlock(48 → 3072 → 768)
                  [6, 512, 768]     컨텍스트 패치 512개
  ↓ [REG] 1개     [6, 513, 768]
  ↓ 미래 패치 3개를 같은 임베딩으로 만들어 뒤에 붙임
                  [6, 516, 768]     ★ 디코더가 없는 이유
  ↓ 인코더 12층 × { TimeSelfAttn(RoPE) → GroupSelfAttn(마스크) → FFN }
                  [6, 516, 768]
  ↓ 뒤에서 3개만 슬라이스
                  [6,   3, 768]
  ↓ ResidualBlock(768 → 3072 → 21×16=336)
  ↓ rearrange "b n (q p) -> b q (n p)"
출력             [6,  21,  48]      변량, 21분위, 48스텝
```

공변량 행(위 예의 1·2번)에 대한 예측도 나오지만 손실에서 제외되고 버려진다 — `inv_future_covariate_mask`가 그 역할이다.

### 3.6 왜 패치를 겹치지 않는가 — PatchTST와 갈리는 지점

*이 절을 두는 이유: PatchTST를 먼저 읽은 사람은 "겹치는 게 정상"이라고 알기 쉬운데, 사전학습 모델 쪽에서는 정반대가 표준이다.*
...
| **MOMENT** | 8 | **8** (`patch_stride_len: 8`) | ❌ |
| **PatchTST 지도학습** | 16 | **8** | ✅ **여기만** |
| **PatchTST 자기지도 사전학습** | 12 | **12** | ❌ |

마지막 두 줄이 핵심이다. 같은 저장소(`yuqinie98/PatchTST`) 안에서 목적에 따라 바뀐다.
...
**PatchTST 저자들도 사전학습을 할 때는 겹침을 없앤다.** 겹침은 "일반적인 것"이 아니라 **소규모 지도학습에 맞춘 선택**이다. (PatchTST 논문이 stride 8을 정당화하지 않는다는 점은 `PAPER_PatchTST.md` Q1 참조.)

#### Chronos가 겹치지 않는 이유 — 세 층위

**(i) 애초에 문제가 없다.** PatchTST가 겹침으로 풀려던 "토큰이 22개뿐"이라는 문제가 존재하지 않는다.

| | 컨텍스트 | P=16, S=16 토큰 수 |
|---|---|---|
| PatchTST | 336 | 22 ← **부족** |
| Chronos-Bolt | 2048 | **128** |
| Chronos-2 | 8192 | **512** |

**(ii) 비용이 정확히 4배다.** stride를 절반으로 줄이면 토큰이 2배, 어텐션은 제곱이므로 4배다.

| 모델 | 겹침 없음 | 50% 겹침 | 어텐션 배수 |
|---|---|---|---|
| Chronos-Bolt (2048) | 128토큰 / 16,384쌍 | 255토큰 / 65,025쌍 | **3.97×** |
| Chronos-2 (8192) | 512토큰 / 262,144쌍 | 1023토큰 / 1,046,529쌍 | **3.99×** |

**(iii) 구조적으로 금지되어 있다 — 이게 결정적이다.** Chronos는 **출력도 패치**라서 겹칠 수가 없다. 코드에 하드 assert가 있다.

```python
# chronos2/model.py:226
...
#                                                          └─ 패치 n개를 끝끝이 이어붙여 horizon 생성
```

```
출력 패치가 겹치지 않을 때        출력 패치가 겹칠 때
[1~16][17~32][33~48]            [1~16]
 └──── 그냥 이어붙임 ────┘           [9~24]    ← 9~16을 두 번 예측. 어느 쪽이 맞나?
```

**출력이 겹칠 수 없으니 patch_size가 같아야 하고, 그래서 입력도 겹치지 않는다.** PatchTST는 출력이 `Flatten → Linear(패치 전부 → 96)` 한 방이라 **출력에 패치 구조가 없고**, 그래서 입력만 자유롭게 겹칠 수 있었다. 이게 두 모델의 진짜 구조적 분기점이다.

**경계 문제는 다르게 해결한다.** "딱 붙여 자르면 경계에 걸친 급변이 어느 패치에도 온전히 안 담긴다"는 우려는 맞지만, ① 학습이 매 샘플마다 시계열의 **무작위 위치**를 잘라내므로(`ExpectedNumInstanceSampler`, `train.py:357`) 패치 격자와 데이터의 위상이 랜덤화되고, ② 컨텍스트가 512패치나 되어 같은 주기 패턴을 수십 번 다른 위상으로 보며, ③ 애초에 50% 겹침도 경계를 8칸마다 남길 뿐 없애지 못한다. 비용 4배를 낼 이유가 없다.

#### 어텐션 비용 실측 (Traffic 862변량, 배치 1개당 쌍 개수)

```
PatchTST (CI, L=336, P=16, S=8)  :   862 × 42²   =    1,520,568
iTransformer (L=96)              :         862²  =      743,044
Chronos-1 (L=512, 변량당 독립)    :   862 × 512²  =  225,968,128
Chronos-2 (L=2048→128패치)
   시간축                         :  862 × 128²   =   14,123,008
   그룹축                         :  128 × 862²   =   95,109,632
```

읽는 법: **Chronos-1이 압도적으로 비싸다** — 패치가 없어서다. 여기에 자기회귀 64스텝 × 샘플 20회가 더 붙는다(forward 1,280회). Bolt의 250배가 이 한 줄에 다 있다. 반대로 **Chronos-2의 그룹축은 iTransformer의 128배**인데, 매 패치 위치마다 862×862를 돌기 때문이다 — iTransformer가 여전히 이기는 지점이 정확히 여기다(실제로는 `group_ids`로 그룹을 작게 쪼개 쓴다).
...
- [PAPER_PatchTST.md](PAPER_PatchTST.md) — Bolt/Chronos-2 패치화의 직계 선조. **단 stride가 다르다**(PatchTST 8 = 겹침 / Chronos 16 = 겹침 없음) — 이유는 §3.6
