...
- **BatchNorm vs LayerNorm(배치 정규화 / 층 정규화)** — PatchTST 인코더는 LayerNorm 대신 BatchNorm을 쓴다(논문 각주로 명시, 근거는 Zerveas et al. 2021).
...
마지막 값 $x_L$을 S번 복제해 뒤에 덧붙인 뒤 자르기 때문에 +2가 된다.
...
**✅ 검증**: 논문 공식과 코드가 정확히 일치. L=336, P=16, S=8 → ⌊320/8⌋+2 = **42** (PatchTST/42). L=512 → ⌊496/8⌋+2 = **64** (PatchTST/64, 제목의 "64 Words").
...
| 정규화 | **BatchNorm** (`norm='BatchNorm'`) | 논문 각주로 명시 ✅ |
...
iTransformer의 메인 표(Table 1)는 **모든 모델의 입력 길이를 96으로 고정**한다. 그런데 **PatchTST의 핵심 주장 자체가 "우리는 긴 look-back을 쓸 수 있다"**(§3.4)이므로, 이 조건은 PatchTST를 자기 최적점 밖에서 평가한 것이다. 옆 문서도 §7.3에서 *"긴 lookback에서 유리해지는 경쟁자(PatchTST, DLinear 계열)와의 비교가 각자 최적 조건이 아닌 지점에서 이뤄진 셈"* 이라고 지적한다.
