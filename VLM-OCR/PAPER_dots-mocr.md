...
**여기서 논문이 말하지 않은 선택이 하나 드러난다 — 임베딩을 일부러 풀었다(untie).** 원본 `Qwen/Qwen2.5-1.5B` 는 `tie_word_embeddings: true` 라 lm_head가 **없다**. 계산이 딱 맞아떨어진다:

```
Qwen2.5-1.5B 원본 = 1,543.7M   (llm 본체 1,310.3M + embed 233.4M, lm_head 없음)
dots.mocr LLM측   = 1,777.1M = 1,543.7M + 233.4M
                                          └─ 새로 붙인 별도 lm_head (config에 tie: false)
```

어휘 151,936 × 1536이라 작지 않은 비용이다(MinerU LLM 본체 357.9M의 **65%** 크기). 구조화 출력 어휘를 따로 학습시키려는 의도로 보이지만 **논문에 이 선택에 대한 언급이 한 줄도 없다.**
...
### Q14. 성능을 높이려고 dots.mocr의 비전 인코더를 MinerU에 이식하면?

*(이어지는 반론: "MinerU도 어차피 두 부품을 갖다 쓴 거잖아, 여기서도 갖다 쓰면 되지 않나?")*

**조립 자체는 정당한 방식이다.** PaliGemma·LLaVA·SmolVLM·nanoVLM 전부 "눈 + 다리 + 입"을 조립한 것이고 MinerU도 그 계보다(→ [[reference_pretrained_backbone_reuse_landscape]]). 문제는 가능/불가능이 아니라 **MinerU의 조립이 유독 쌌던 이유**다.

#### MinerU의 조립은 "같은 가문 안에서"였다

| 정렬 항목 | MinerU (Qwen2-VL ViT + Qwen2-0.5B) | dots 인코더 + Qwen2-0.5B |
|---|---|---|
| 인코더가 원래 붙어 있던 LM | **Qwen2-1.5B** (같은 가문) | Qwen2.5-1.5B (다른 세대) |
| vocab / 토크나이저 | 151,936 **동일** | 151,936 **동일** ✅ |
| 위치 인코딩 규약 | **M-RoPE 그대로** | dots는 1D RoPE → M-RoPE로 재정의 필요 |
| 이미지 전처리 규약 | **동일**(temporal_patch=2, Conv3d) | 다름(temporal_patch=1, Conv2d) |
| merger 출력 차원 | 1280×4 → **896** (원래부터 맞춰짐) | 1536×4 → 896으로 **새로 학습** |
| 필요한 alignment | 이미 80% 되어 있음 → **재조정** | 인터페이스 신규 → **처음부터** |

핵심: **Qwen2-VL-2B의 ViT는 이미 Qwen2 LM과 함께 학습된 상태로 출고됐다.** MinerU는 그 커플에서 LM만 1.5B → 0.5B로 갈아 끼운 거라 "말이 통하는 상태"를 대부분 물려받았고, LLaVA-Pretrain급 가벼운 Stage 0로 끝났다. 반면 dots 인코더는 **dots 자기 LM(1536차원)하고만 말이 통한다.** 불가능이 아니라 **비용 문제**다.

#### 그래도 하겠다면 — 실제 절차

vocab이 같아서 생각보다 할 만하다:

```
1. merger 마지막 층만 교체: [1536, 6144] → [896, 6144]      (5.5M 새로 초기화)
2. 이미지 프로세서를 temporal_patch=1 기준으로 통일,
   LM 쪽 M-RoPE 위치 id 생성 로직 수정
3. Stage 0: 인코더·LM 동결, merger만 학습                   (LLaVA-Pretrain급)
4. Stage 1: 인코더 unfreeze, 문서 파싱 데이터로 SFT          ← 진짜 비용
```

3번까지는 GPU 며칠. **문제는 4번** — MinerU2.5는 6.9M×2ep + 630K×3ep, Pro는 65.5M을 썼는데 **데이터도 학습 코드도 비공개**다. 본인 데이터로 해야 하고 그 품질이 곧 성능이 된다.

#### 조합을 바꾸면 훨씬 싸다

원하는 게 **"dots의 좋은 눈 + 작은 입"** 이라면, 눈을 MinerU로 옮기지 말고 **dots 쪽에서 입을 줄이는 게 반값**이다:

| 방향 | 바꾸는 것 | 남는 것 | 비용 |
|---|---|---|---|
| MinerU에 dots 눈 달기 | 인코더 + 전처리 규약 + merger | MinerU 학습 데이터(비공개, 못 씀) | 정렬 + 전체 재학습 |
| **dots에서 입 줄이기** | LM만 1.5B → 0.5B급, merger 출력 1536→896 | **인코더가 자기 학습 분포(native 고해상도) 안에 머묾**, 전처리·파이프라인 그대로 | 정렬 + SFT |

후자는 **3.04B → 약 1.76B**(비전 1,262M + LM 494M), 가중치 6.1GB → 3.5GB. 그리고 **이건 논문의 검증 안 된 주장을 검증하는 실험이기도 하다** — §3.2가 "1.5B보다 훨씬 작으면 긴 구조화 출력을 감당 못 한다"고 단정하는데 **근거 ablation이 0건**이고, MinerU가 0.5B로 문서 파싱 SOTA를 찍은 것 자체가 반례에 가깝다.

#### 다만 "무엇이 좋아지나"의 근거는 여전히 약하다

- MinerU2.5-Pro 자체 ablation에서 최대 기여는 **순수 데이터 확장 +1.31**, 저자 결론은 "병목은 architecture가 아니라 data".
- dots 인코더의 강점은 "전체 페이지를 통째로"인데 MinerU 파이프라인은 **의도적으로 축소판만 보여준다.** 좋은 눈을 달고 축소판을 보게 하는 셈.

#### 성능을 올리려면 — 더 나은 3가지

**A. 방향을 반대로: MinerU의 2단계를 dots.mocr에 이식** ← 가장 실용적

```
dots.mocr `prompt_layout_only_en` 로 박스만        (짧은 출력 = 잘림 없음)
        ↓ 박스대로 원본에서 crop
dots.mocr `prompt_grounding_ocr` 로 조각별 읽기     (병렬)
        ↓ 생성 순서대로 재조립
```

**학습이 전혀 필요 없다.** 두 프롬프트 모두 이미 학습돼 있다. dots.mocr 최대 약점(수만 토큰 단일 시퀀스 → 잘림·반복 붕괴 → JSON 깨지면 페이지 통째 손실, → Q13)이 사라지고 강점(텍스트 90.0, 열화 스캔 수식 85.5)은 남는다. 대가는 호출 횟수와 crop 경계 문맥 손실.

**B. CNN 검출기 + dots.mocr 인식** — 박스가 결과물인 용도면 전용 검출기로 박스를 잡고 `prompt_grounding_ocr`로 읽는다(→ Q12 처방 2).

**C. 페이지 유형별 라우팅** — 깨끗한 디지털 PDF는 MinerU, 열화 스캔·수식 밀집은 dots.mocr. 두 모델을 섞는 게 한 모델을 개조하는 것보다 훨씬 싸다.

**정말 MinerU를 키우고 싶다면** 인코더 교체보다 **Stage I 축소판 해상도를 1036에서 올리는 것**이 비교할 수 없이 싸다. `layout_image_size=(1036,1036)` 한 줄이고 재학습 없이 즉시 측정된다.

### Q15. LM을 더 큰 Qwen(2~4B)으로 키우면 좋아지지 않나?

#### 반증 먼저 — 문서 파싱은 파라미터와 점수의 상관이 거의 없다

MinerU2.5-Pro 논문의 OmniDocBench v1.6 표를 크기순으로 정렬하면:

| 모델 | 파라미터 | Full ↑ |
|---|---|---|
| **MinerU2.5-Pro** | **1.2B** | **95.69** |
| GLM-OCR | 0.9B | 95.15 |
| PaddleOCR-VL-1.5 | 0.9B | 94.87 |
| Youtu-Parsing | 2.5B | 93.68 |
| Ovis2.6-30B-A3B | 30B | 93.62 |
| Logics-Parsing-v2 | 4B | 93.27 |
| Gemini 3 Pro | – | 92.85 |
| dots.ocr | 3B | 90.50 |
| **Qwen3-VL-235B** | **235B** | **89.78** |
| InternVL3.5-241B | 241B | 83.61 |

**0.9B가 235B를 5.4점 이긴다.** 200배 차이인데 결과는 반대다.

#### 왜 그런가 — LM 크기는 독립 손잡이가 아니라 추론 설계에 종속된 변수다

MinerU가 0.5B로 충분한 건 **설계와 짝을 이뤄서**다. Stage II는 **crop 하나 → 짧은 텍스트 하나**라 긴 문맥 유지도, 수만 토큰 생성도, 블록 간 참조도 LM이 할 일이 아니다(그건 Stage I 레이아웃과 후처리 규칙 담당). 이 상태에서 LM만 키우면 **같은 일을 3배 비싸게 할 뿐**이다. 반대로 dots.mocr이 1.78B를 쓰는 건 페이지 통째로 수만 토큰 JSON을 뱉어야 해서다.

즉 **두 팀 모두 "자기 추론 설계에 맞는 LM 크기"를 고른 것**이지 한쪽이 인색했던 게 아니다. MinerU에서 LM을 키우는 게 의미를 가지려면 **설계를 먼저 바꿔야** 한다.

#### 그래도 키우면 이득이 날 만한 곳 — 두 군데

1. **긴 구조 시퀀스 생성** — 표 OTSL이 대표적. MinerU2.5-Pro의 표 TEDS 93.42는 종합 1위지만 **Base 구간에서는 GLM-OCR(0.9B)에 밀린 2위**다. 긴 수식은 지금 **ADR로 줄 단위로 쪼개서** 처리하는데, 큰 LM이면 안 쪼개도 될 수 있다.
2. **다국어** — MinerU의 약한 축. 저자원 언어는 LM에 그 언어가 얼마나 들어 있느냐가 그대로 성능이라 0.5B는 불리하다.

#### 이 실험은 두 논문이 공통으로 빼먹은 ablation이다

| 논문 | 주장 | 근거 |
|---|---|---|
| dots.mocr §3.2 | "1.5B보다 훨씬 작으면 긴 구조 출력을 못 다룬다" | **ablation 0건** |
| MinerU2.5 | "문서 파싱은 대규모 언어모델 의존도가 낮다" | **ablation 0건** |

**정반대 주장인데 양쪽 다 근거가 없다.** 이걸 끝낼 실험이 정확히 "파이프라인 고정, LM만 교체"다.

#### 후보 목록 (HF config 실측)

| 모델 | 파라미터 | hidden | 층 | heads Q/KV | intermediate | vocab | tie | max_pos | 라이선스 |
|---|---|---|---|---|---|---|---|---|---|
| Qwen2-0.5B *(MinerU 현재)* | 0.494B | 896 | 24 | 14/2 | 4864 | 151,936 | ✅ | 131,072 | Apache-2.0 |
| Qwen2.5-0.5B | 0.494B | 896 | 24 | 14/2 | 4864 | 151,936 | ✅ | 32,768 | Apache-2.0 |
| **Qwen2.5-1.5B** *(dots.mocr 현재)* | **1.544B** | 1536 | 28 | 12/2 | 8960 | 151,936 | ✅ | **131,072** | Apache-2.0 |
| **Qwen3-1.7B-Base** | **1.721B** | 2048 | 28 | **16/8** | 6144 | 151,936 | ✅ | 32,768 | Apache-2.0 |
| Qwen2.5-3B | 3.086B | 2048 | 36 | 16/2 | 11008 | 151,936 | ✅ | 32,768 | ⚠️ **`other`** |
| **Qwen3-4B-Base** | **4.022B** | 2560 | 36 | **32/8** | 9728 | 151,936 | ✅ | 32,768 | Apache-2.0 |
| Qwen2.5-7B | 7.616B | 3584 | 28 | 28/4 | 18944 | **152,064** | ❌ | 131,072 | Apache-2.0 |

**vocab이 Qwen2.5-7B만 빼고 전부 151,936으로 같다** → 토크나이저를 그대로 쓸 수 있어 이 범위 안 교체는 생각보다 싸다.

#### 함정 3가지

**1. Qwen2.5-3B는 라이선스가 다르다** ⚠️ — Qwen2.5 시리즈에서 **3B와 72B만 Apache-2.0이 아니다**(`license: other` = Qwen Research License). 0.5B/1.5B/7B/14B/32B는 Apache-2.0. "2~4B" 구간의 가장 자연스러운 후보가 하필 여기 걸리므로, 같은 구간이면 **Qwen3-4B-Base**를 쓸 것.

**2. Qwen3는 KV cache가 4배 비싸다 — dots.mocr에 치명적.** Qwen3는 GQA의 **KV head가 2 → 8**로 늘었다:

| LM | KV/토큰 | dots.mocr 페이지 1장(14,400토큰) | MinerU Stage I(1,369토큰) |
|---|---|---|---|
| Qwen2-0.5B | 12 KB | – | **16 MB** |
| Qwen2.5-1.5B | **28 KB** | **403 MB** | 38 MB |
| Qwen3-1.7B | **112 KB** | **1.61 GB** | 153 MB |
| Qwen3-4B | **144 KB** | **2.07 GB** | 197 MB |

→ 역설적이지만 **"Qwen 상위 버전 갈아타기"는 dots.mocr보다 MinerU에 훨씬 잘 맞는다.**

**3. Qwen3-Base는 컨텍스트가 32K다.** Qwen2.5-1.5B는 131,072인데 Qwen3-Base는 32,768. dots.mocr은 이미지 토큰 최대 14,400 + 출력 최대 32,768 = **47K가 필요**해 32K를 넘긴다(YaRN 확장은 가능하나 추가 작업). **dots.mocr이 Qwen2.5-1.5B를 고른 진짜 이유가 여기일 가능성이 크다** — 131K 컨텍스트가 페이지 통째 파싱에 필요했던 것. 논문은 이 얘기를 하지 않는다.

#### 대상별 결론

- **MinerU를 키운다면 → `Qwen3-1.7B-Base`**: vocab 동일, Apache-2.0, KV 증가분이 절대량으로 작음(16MB → 153MB). 노리는 이득은 긴 표 OTSL·긴 수식·다국어. merger 마지막 층만 896 → 2048로 교체.
- **dots.mocr을 키운다면 → 권하지 않음**: Qwen3로 가면 KV 403MB → 1.6GB에 컨텍스트도 131K → 32K로 줄어 오히려 손해. 같은 세대 안의 Qwen2.5-3B는 라이선스가 MIT 배포를 막는다. **줄이는 쪽(1.5B → 0.5B)이 더 값싼 검증 실험**이다(→ Q14).
- **공통**: 어느 교체든 merger 재학습 + SFT가 필요하다. vocab이 같아 토크나이저는 공짜지만 "가중치만 갈아 끼우면 되는" 작업은 아니다.

### Q16. 다른 문서 OCR 논문들과 어떤 관계인가?
