...
| 공개 범위 | **추론 코드 + 체크포인트만.** 학습 코드·학습 데이터·Data Engine 구현 전부 비공개 |
...
2. **데이터 구성 비율의 변화**가 의도를 드러냅니다.
   ...
   미세조정에서 레이아웃 비중이 33% → 7%로 급감합니다. **레이아웃은 사전학습에서 거의 끝나고, 어려운 건 내용 인식**이라는 판단입니다.
...
MinerU2 파이프라인으로 초벌 주석을 만든 뒤, 유형별 전문 모델로 덮어씁니다.

| 유형 | 정제에 쓴 모델 |
|---|---|
| 텍스트 | **Qwen2.5-VL-72B-Instruct**로 crop별 인식 결과를 검증·교정 |
| 수식 | 자체 데이터로 재학습한 **UniMERNet** 출력으로 치환 |
| 표 | 자체 고성능 표 파싱 모델로 구조 전면 재생성 |
