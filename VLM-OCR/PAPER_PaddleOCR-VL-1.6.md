...
### Q11. 세 논문의 학습셋 구성은? 공개했나?

*세 논문이 입을 모아 "이제 데이터가 병목"이라고 결론 내렸으니, 정작 그 데이터를 얼마나 밝혔는지가 이 분야의 실질적 진입 장벽을 결정한다.*

#### ① 공개 여부 — HuggingFace 계정을 직접 확인한 결과

| 조직 | 공개 데이터셋 | 문서 파싱 **학습** 데이터 |
|---|---|---|
| **opendatalab** (MinerU) | 31개 — `OmniDocBench`(3.3만 DL), `OHR-Bench`, `WebMainBench`, `ChartVerse-SFT-1.8M` 등 | ❌ **없음** |
| **PaddlePaddle** | 6개 — `Real5-OmniDocBench`(3.4천 DL), `PaddleOCR-VL_demo`(데모 이미지) | ❌ **없음** |
| **zai-org** (GLM) | 30개 — 전부 LongBench · SWE · Agent 계열 | ❌ **OCR 관련 데이터셋 자체가 0개** |

공개된 건 **전부 평가용**이다. OmniDocBench와 Real5는 벤치마크지 학습셋이 아니고, `PaddleOCR-VL_demo`는 데모 이미지 몇 장이다. 즉 세 논문 모두 **"데이터가 병목이다"라고 주장하면서 그 데이터는 내놓지 않았다.**

#### ② 🥇 MinerU2.5-Pro — 유일하게 분해까지 공개
...
| 수식 인식 | **13M** | 20% |
| 표 인식 | **11.5M** | 18% |
| 이미지 분석 (차트, 텍스트 삽입 이미지 등) | **6M** | 9% |

**Stage 2 (3.9M)** — 그중 **192K가 사람 라벨**, 나머지는 replay / **Stage 3 (192K)** — GRPO

여기에 **난이도 분포**까지 밝힌다 — CMCV 기준 Easy 약 60% / Medium 약 25% / Hard 약 15%. 파이프라인은 "PDF Pool → DDAS 샘플링 → 페이지 수준 약 60M → 요소 수준 추출". baseline인 MinerU2.5는 **전체 10M 미만**(Stage 1은 6.9M × 2 epoch)이었으니, Pro가 한 일은 **약 10배 증량**이다.

⚠️ **다만 걸리는 문장이 하나 있다.**

> "서브태스크 비율은 **OmniDocBench 종합 점수에서 각 태스크가 갖는 가중치**와 baseline 모델의 태스크별 성능 격차에 맞춰 조정했다."

즉 **벤치마크 점수 가중치를 보고 데이터 비율을 역산했다.** 게다가 그 벤치마크(v1.6)를 자기들이 만들었다. 정직하게 밝힌 건 좋지만, "데이터 엔지니어링의 승리"와 "벤치마크 조준"이 얼마나 분리되는지는 판단하기 어렵다.

#### ③ 🥈 PaddleOCR-VL-1.6 — 총량만, 분해 없음

| 단계 | 규모 | 구성 설명 |
|---|---|---|
| CPT | **16.8M** | 1.5의 SFT 전량 + 사전학습 데이터 **일부** + 신규 검색 데이터 전량 |
| SFT | **7.3M** | UACS 하드샘플 + 전문가 합의 실패분(Algorithm 2 통과) + 라벨 교정분 |
| RL | **49K** | 태스크별 상위 8K |

서브태스크별 분해가 **없다.** 데이터 출처는 "massive internal document pool"이라고만 하고, 신규 보강한 **유형**만 언급한다 — 고서(ancient books), 희귀 한자(rare characters), 산업용 표(industrial tables). **사람 주석 규모도 안 밝힌다** — Algorithm 2가 "끝까지 안 풀리면 사람에게 넘긴다"고 하는데 그게 몇 건인지 알 수 없다.

#### ④ 🥉 GLM-OCR — 숫자가 사실상 없음

| 단계 | 데이터 |
|---|---|
| Stage 1 (인코더) | "**수백억(tens of billions) image-text 쌍**" ← 논문 전체에서 유일한 규모 숫자 |
| Stage 2.1 | Image-text pairs, Document parsing, Grounding, VQA |
| Stage 2.2 | Document parsing, Grounding, VQA |
| Stage 3 (SFT) | Text / Formula / Table recognition, KIE |
| Stage 4 (RL) | 동일 |

**타입만 나열하고 규모는 전부 공란이다.** Table 1이 "Training Recipe"라는 제목인데 실제로는 데이터 종류 목록이고, 배치·LR·에폭도 없다. 세 논문 중 학습 정보가 가장 적다.

#### ⑤ 한눈에 비교

| | PaddleOCR-VL-1.6 | GLM-OCR | MinerU2.5-Pro |
|---|---|---|---|
| 총 학습 규모 | 16.8M / 7.3M / 49K | ❌ | 65.5M / 3.9M / 192K |
| **서브태스크 분해** | ❌ | ❌ | ⭕ **5개 항목** |
| **사람 주석 규모** | ❌ | ❌ | ⭕ **192K** |
| 난이도 분포 | ❌ | ❌ | ⭕ 60 / 25 / 15 |
| replay 비율 | ❌ | ❌ | ⭕ 태스크별 (1:50 ~ 6:1) |
| 데이터 출처 | "내부 풀" | ❌ | "PDF Pool" |
| 학습 하이퍼파라미터 | 부분 (LR, batch) | ❌ | ⭕ 3단계 전부 |
| **데이터 자체 공개** | ❌ | ❌ | ❌ |
| 벤치마크 공개 | ⭕ Real5 | ❌ | ⭕ **OmniDocBench v1.6** |

#### ⑥ 그래서 실제로 건질 수 있는 것 3가지

데이터를 참고할 방법은 없고, 참고할 수 있는 건 **비율과 방법론**뿐이다. 전부 MinerU2.5-Pro에서 나온다.

1. **서브태스크 비율** — 텍스트 32% / 레이아웃 21% / 수식 20% / 표 18% / 이미지 9%. 데이터를 직접 만들 때 어디에 몇 퍼센트를 쓸지의 출발점. **텍스트가 3분의 1**이라는 게 핵심.
2. **replay 비율** — 하드샘플 SFT에서 텍스트는 1:50, 레이아웃은 6:1 (→ Q8).
3. **난이도 분포 기대치** — CMCV를 돌렸을 때 Easy 60 / Medium 25 / Hard 15가 안 나오면 내 파이프라인이 이상한 것. 진단 기준선으로 쓸 수 있다.

공개 자원 중 실제로 대체재가 될 만한 것:

| 용도 | 후보 | 주의 |
|---|---|---|
| 표 | PubTabNet, FinTabNet | 도메인 편중(논문 · 재무) |
| 수식 | UniMERNet, im2latex 계열 | GLM-OCR · MinerU도 이걸로 평가함 |
| 레이아웃 | DocLayNet, PubLayNet | 카테고리 체계가 서로 다름 |
| 차트 | `opendatalab/ChartVerse-SFT-1.8M` (공개됨) | MinerU 학습셋인지는 논문에 언급 없음 |
| 스캔 원본 | IIT-CDIP 등 | **라벨 없음** — 합의 라벨을 붙여야 함 |
| 평가 | OmniDocBench v1.6, Real5 | ⚠️ **학습에 절대 넣지 말 것** |

**한 줄**: 세 논문 모두 "이제 구조가 아니라 데이터가 병목"이라고 결론 내렸는데, **그 데이터를 공개한 팀은 하나도 없다.** MinerU2.5-Pro만이 구성 비율 · 사람 주석 규모 · replay 비율을 밝혀 **레시피는 따라 할 수 있게** 해뒀고, 나머지 둘은 결과만 보고한다. → 이 제약 아래에서 실제로 모델을 만들려면 어떻게 해야 하는지는 별도 문서 `DESIGN_DocParser-Scratch.md` 참조.
