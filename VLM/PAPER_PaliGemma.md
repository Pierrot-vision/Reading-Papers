...
- **후속**: PaliGemma 2(2024-12)는 언어 백본을 Gemma 2(2B/9B/27B)로 확장하고 해상도/규모 조합을 넓힘. HF `PaliGemmaForConditionalGeneration`은 두 세대를 공통 클래스로 지원. → 상세 정리 및 1 vs 2 비교표: [PAPER_PaliGemma-2.md](PAPER_PaliGemma-2.md)
...
공개했다. 다만 층위를 나눠 봐야 한다.

| 항목 | 공개 여부 |
|---|---|
| 파인튜닝 코드 (JAX, big_vision) | ✅ |
| 추론 코드 | ✅ |
| 학습 config·레시피 | ✅ |
| 베이스/파인튜닝 가중치 | ✅ (HF·Kaggle, Gemma 라이선스) |
| HF transformers 파인튜닝 경로 | ✅ (PyTorch 사용자용 사실상 표준) |
| 대규모 사전학습 **데이터** | ❌ (WebLI 등 비공개) |

- **공식 코드**: [google-research/big_vision](https://github.com/google-research/big_vision) 레포의 [`configs/proj/paligemma`](https://github.com/google-research/big_vision/tree/main/big_vision/configs/proj/paligemma). **JAX + Flax**, 데이터는 **TFDS**, TPU 학습 전제.
- **범위**: 파인튜닝 + 추론 + config 공개 → **Stage 3(내 과제 전이)는 그대로 재현 가능**.
- **주의**: 대규모 사전학습(Stage 1·2)은 코드·config는 있어도 **웹 규모 사설 데이터(WebLI 등) + TPU 자원** 때문에 처음부터 재현은 사실상 불가. 논문의 전제는 "가중치를 받아 **Stage 3만 재현**".
- **실무 경로**: JAX 스택이 부담이라, PyTorch 사용자는 **HF `transformers`의 `PaliGemmaForConditionalGeneration`**으로 파인튜닝하는 게 사실상 표준([공식 노트북](https://github.com/ariG23498/fine-tune-paligemma), NielsRogge 튜토리얼).

한 줄: **"코드·가중치·레시피는 공개, 사전학습 데이터는 비공개."** [[paper_i1]](완전공개 지향)·[[paper_firered_image_edit]](SFT까지 재현)와 같은 "공개 범위" 관점.

### Q12. logit soft-capping이 뭔가?

**로짓(logit) 값이 너무 커지지 않게 부드럽게 눌러주는(cap) 학습 안정화 안전장치.** Gemma 2 계열(따라서 **PaliGemma 2**)에서 쓴다. logit = softmax 직전 각 후보(단어/어텐션 위치)의 "점수"인데, 학습 중 이 값이 폭주하면 불안정해진다.

**공식 (말로 풀면)**: `softcap(x) = cap × tanh(x / cap)`

- x를 cap으로 나눠 **tanh**(−1~1로 눌러주는 S자 함수)에 통과시킨 뒤 다시 cap을 곱함 → 값이 항상 **−cap ~ +cap 사이**.
- 작은 값은 거의 그대로 통과(x 작으면 softcap(x)≈x), **큰 값만 cap 근처에서 완만히 눌림**.

**왜 "soft"인가 (hard clipping과 비교)**:

| | hard clipping (딱 자르기) | soft-capping (부드럽게 누르기) |
|---|---|---|
| 방식 | cap 넘으면 그냥 cap으로 싹둑 | tanh로 완만하게 수렴 |
| 미분(gradient) | 잘린 구간 기울기 0 → 학습 신호 끊김 | 어디서나 매끄럽게 미분 가능 → 학습 신호 유지 |

**어디에 쓰나 (Gemma 2 기준)**: 두 군데에 각각 cap.
- **attention logit soft-capping** (cap ≈ **50**): 어텐션 점수 폭주 방지.
- **final logit soft-capping** (cap ≈ **30**): 마지막 출력 로짓 폭주 방지.

**PaliGemma 맥락 주의점**:
- PaliGemma(1)은 Gemma **1** 백본이라 원래 soft-capping **없음**. **PaliGemma 2**(Gemma 2 백본)부터 들어옴.
- 실무 함정: soft-capping은 **Flash Attention 등 고속 어텐션 커널과 호환 안 되는** 경우가 있어, 속도를 위해 **끄는(disable)** 경우가 많음. HF에서 학습이 느리거나 경고가 뜨면 이 설정 확인.

한 줄: **tanh로 로짓을 −cap~+cap에 부드럽게 가둬 폭주를 막는 안정화 트릭.** Gemma 2/PaliGemma 2의 attention(50)·final(30) 두 곳에 적용.
