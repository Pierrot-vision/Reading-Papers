...
> 왜? 같은 "입력 이미지"라도, 편집 대상은 **픽셀까지 그대로 보존**해야 하고 참조 인물은 **특징(정체성)만** 가져오면 된다. 둘을 같은 위치 좌표로 넣으면 모델이 역할을 혼동한다. 좌표 부여 규칙 하나로 이 둘을 분기시키는 게 핵심.
>
> ※ 아래 상대-offset 계산은 RoPE 원리에서 따라오는 해석이고, 논문 자체는 "충실도 요구에 따라 좌표를 조정하는 task-specific prior"라는 큰 방향만 명시한다.

**전제 — RoPE는 절대 좌표가 아니라 "상대 위치(좌표 차이)"를 본다.** 2D RoPE(2차원 회전 위치 임베딩)에서 두 토큰의 attention(어텐션)은 두 토큰 좌표의 **차이**로 정해진다. 그래서 "어디에 두느냐"보다 **"생성 중인 타겟 픽셀과 입력 픽셀의 상대 위치가 어떻게 잡히느냐"** 가 본질이다.

**세 종류 토큰의 좌표** (latent 격자: 가로 w개, 세로 h개 토큰, i∈[0,w−1] 가로, j∈[0,h−1] 세로):
- **target(생성될 출력)**: 기준 격자 (i, j) — 원점에서 시작하는 정상 1사분면.
- **source(편집 대상)**: (i′ₛᵣc, j′ₛᵣc) = **(−i, −j)** — target을 원점 기준으로 뒤집은(점대칭) 음수 사분면.
- **reference(n번째 참조)**: (iⁿ_ref, jⁿ_ref) = **(i + w·n, j + h·n)**, n∈[1,N] — 1사분면 바깥으로 대각선 한 칸(w,h)씩 밀린 별도 타일.
...
```
        j (세로)
          ...
          │            · ref2  (2w~3w, 2h~3h)
          │        · ref1  (w~2w, h~2h)
          │    □ target (0~w-1, 0~h-1)   ← 기준
 ─────────┼─────────────────────────→ i (가로)
  ■ source│
  (−i,−j) │  (target을 원점 기준 점대칭으로 뒤집은 자리)
```

**핵심 — 타겟 픽셀과 입력 픽셀의 상대 위치를 직접 계산** (격자상 같은 칸 i, j끼리 비교):

- **source(편집): 상대 = (2i, 2j) → 픽셀마다 다름.** 타겟 (i,j) vs source (−i,−j) → 차이 = (i−(−i), j−(−j)) = **(2i, 2j)**. i,j에 따라 **위치마다 고유**하므로, 타겟의 각 픽셀이 source의 특정 픽셀과 **일대일 결정적으로 대응**된다("(i,j)를 채우려면 source의 (−i,−j)를 보라"). → **per-pixel 공간 대응 = 편집의 픽셀 충실도(pixel fidelity)** 토대.
- **reference(참조): 상대 = (−w·n, −h·n) → 모든 픽셀이 동일 상수.** 타겟 (i,j) vs n번째 ref (i+w·n, j+h·n) → 차이 = **(−w·n, −h·n)**. i,j가 사라져 **어느 픽셀에서 보든 같은 상수**다. 픽셀별 대응이 없으니 "위치를 베끼는" 게 아니라 **참조를 통째로 정체성/내용으로만** 반영 → **identity preservation(정체성 보존)** 토대.

**다중 참조 구분**: 상대 위치 (−w·n, −h·n)이 **n마다 다른 상수**(ref1=(−w,−h), ref2=(−2w,−2h)…)라, 모델이 위치만으로 "몇 번째 참조"인지 구분한다. 타일도 대각선으로 떨어져 **서로 절대 겹치지 않아** 여러 장이어도 내용이 안 섞인다.

**왜 source를 target과 좌표 공유(차이 0) 안 했나?** 픽셀 충실도만 보면 source를 target에 겹쳐도 되지만(FLUX-Kontext류), 그러면 "이 위치 토큰이 source인지 target 이웃인지" **출처가 헷갈린다.** (−i,−j)로 음수 사분면에 분리하면 **출처는 명확히 갈라놓으면서도** (2i,2j)라는 **픽셀별 결정적 대응은 유지** — "분리 + 대응"을 동시에 얻는다.

**검증 (논문 Table 9)**: 같은 입력을 source로 다룰 때 DINO **0.865** / CLIP-I **0.914**, reference로 다룰 때 **0.786** / **0.858** — 픽셀 보존이 필요한 편집에서 source 방식이 정체성 유사도까지 더 높게 나와 설계가 실제로 작동함을 확인.
...
#### 왜 MLLM을 동결만 안 하고 LoRA까지 붙였나? (관행과의 차이)

> 왜? 통합 생성 모델의 **흔한 관행은 VLM을 그냥 동결**해 조건 인코더로만 쓰는 것인데, 이 논문은 굳이 LoRA를 얹어 살짝 학습시킨다. 이 선택이 이 모델의 특이점이라 짚고 넘어간다.

**관행 — VLM = 수동적 이해 인코더**: 대다수 모델은 VLM/LLM을 동결해 표현만 빼서 쓴다. Ovis-Image는 이해용 LLM을 동결해 텍스트 인코더로만 쓰고, MetaQuery류도 VLM은 freeze한 채 learnable query + connector만 학습한다. "표현은 이미 충분히 좋으니 그대로 쓰면 된다"는 관점.

**Query-Kontext — VLM = 능동적 생성 추론기**: 이 논문은 VLM에게 더 무거운 일을 시킨다 — 단순 이해를 넘어 **여러 참조 이미지 + 편집 지시를 종합·추론해 "무엇을 어떻게 그릴지"를 128개 kontext 쿼리 토큰이라는 새 출력 형식으로 능동적으로 합성**하는 일이다. 이건 사전학습 때 안 해본 과제이고, 특히 **kontext 토큰(학습형 쿼리)은 VLM이 본 적 없는 새 입력**이라 attention이 이 쿼리를 잘 읽어 의미를 채우려면 내부 동작을 약간 틀어줘야 한다. Connector(외부 통역사)만으론 못 메우는 이 부분을, **VLM 내부 attention을 LoRA로 살짝 조정**해 해결한다. 논문 표현으로는 *"VLM에 잠재된 생성적 추론 능력을 끌어내되(unleash generative reasoning), 원래 이해 능력은 보존(preserving understanding)"* — 동결로는 "끌어내기(unleash)"가 안 되는 게 핵심이다.

**반전 — 결국 다시 동결된다**: 그럼에도 LoRA 학습은 **Stage 1에서만** 하고, 이후 본체에 병합해 동결한다. 즉 *"처음에 LoRA로 한 번 생성 추론에 길들인 뒤 → 고정"*. 그래서 2·3단계 추론 시점의 최종 상태는 일반적인 동결 VLM과 같아진다. 관행(처음부터 freeze)과 본 논문(LoRA로 깨운 뒤 freeze)의 차이는 **"동결 전 워밍업이 있느냐"** 한 가지다.

| 접근 | VLM 역할 | VLM 학습 | 예 |
|---|---|---|---|
| 동결 인코더 (관행) | 수동적 이해 인코더 | 동결만 | Ovis-Image · MetaQuery류 |
| **LoRA로 깨운 뒤 동결 (Query-Kontext)** | 능동적 생성 추론기 | **Stage 1만 LoRA → 병합·동결** | 본 논문 |

(참고: full 학습을 안 쓴 이유는 별개 — 7B 전체를 풀면 사전학습 이해 능력이 망가질 위험(catastrophic forgetting, 치명적 망각)과 비용이 크기 때문. 그래서 "동결도 부족, full도 과함 → LoRA"라는 절충.)
