...
- **Connector (커넥터)**: MLLM이 뱉은 토큰을 diffusion이 알아먹는 latent 공간으로 변환하는 **2층 MLP(~50M 파라미터)**.
...
핵심은 **MLLM의 언어 출력은 버리고 kontext 토큰만 쓴다**는 점(Fig.2의 "Discarded"). VLM을 "텍스트 생성기"가 아니라 "조건 인코더"로 재정의한 것이다.
...
### Q2. 왜 MLLM의 언어 출력을 버리나(Discarded)?

이 모델의 목표는 텍스트 생성이 아니라 **조건 토큰 추출**이다. VLM은 멀티모달 입력을 깊이 이해하는 능력만 빌려 쓰고, 그 이해를 128개 kontext 토큰에 담아 diffusion으로 넘긴다. 그래서 다음 단어 예측용(언어 모델링) 출력은 불필요해 버린다 — VLM을 "텍스트 기계"에서 "조건 인코더"로 재정의한 것.

### Q3. 사전학습 백본 재사용 풍경에서 어디에 속하나?
