...
학습은 **T2I + TI2I + I2I(재구성)** 멀티태스크로 묶어 Qwen2.5-VL 과 MMDiT 의 잠재 공간을 정렬합니다. 핵심 통찰: **"의미는 VLM 이, 디테일은 VAE 가"** 역할 분담 — 둘 다 넣어야 균형이 잡힘(실증). novel view synthesis(새 시점 합성)·depth estimation(깊이 추정) 같은 비전 태스크도 모두 "편집의 일종"으로 이 틀에 흡수됩니다.
