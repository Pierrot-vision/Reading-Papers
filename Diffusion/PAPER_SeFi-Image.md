...
| **SemVAE 코드** | ❌ 이 레포엔 없음 (SFD 레포 `tokenizer/semvae/` 코드를 씀) |
| **SemVAE 가중치** | ✅ **2026-07-04 별도 공개** — HF [SeFi-Image/SeFi-Image-SemVAE](https://huggingface.co/SeFi-Image/SeFi-Image-SemVAE) (`dinov2_vitl14_reg/transformer_ch16`, 100만 스텝, cc-by-nc-4.0). `latent_dim`=16 확정 |
| **DINOv2, REPA, 학습 loss** | ❌ 없음 (diffusion 본체 학습 코드 미공개) |
...
- Semantic: **16채널** (공개된 SemVAE 체크포인트 `latent_stats.pt`의 `latent_dim`=16로 확정. `dinov2_vitl14_reg/transformer_ch16`)
- 최종 latent는 `(B, 16+128, H/16, W/16)` **하나의 텐서**. 두 스트림이 같은 공간 격자·같은 RoPE 좌표(img_ids)를 공유하는 **토큰별 채널 결합**
...
**있다. 단 "추론 쪽 절반"만.** 스케줄(runner.py)·dual timestep 임베딩(flux2_sefi_transformer.py)·채널 concat은 구현돼 있고, diffusion 본체 학습 쪽(DINOv2 추출, REPA, loss)은 미공개. **단 SemVAE는 예외** — 코드는 SFD 원논문 레포(`tokenizer/semvae/`)에 있고, 가중치는 2026-07-04 HF [SeFi-Image/SeFi-Image-SemVAE](https://huggingface.co/SeFi-Image/SeFi-Image-SemVAE)로 별도 공개됐다(`dinov2_vitl14_reg` ch16, cc-by-nc-4.0). 상세 매핑과 clamp 기반 마스크 구현의 발견은 → **알고리즘 3절 참조**. 재현 가능 범위는 FireRed-Image-Edit와 비슷한 반쪽 공개 패턴("추론 + SemVAE까지, diffusion 본체 학습은 비공개").
