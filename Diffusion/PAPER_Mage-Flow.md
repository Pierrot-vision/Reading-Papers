...
**한 줄로**: 노이즈에서 목표까지 가는 길을 **여러 번 나눠서 걸어가는** 방식입니다.
...
> ⚠️ **헷갈리기 쉬운 점 — "멀티스텝으로 학습한다"는 말의 진짜 뜻**
>
...
>
> 그러면 논문의 "multi-step flow-matching pre-training"은 무슨 뜻일까요? **"추론할 때 멀티스텝으로 써야 하는, 평범한 확산 모델로 학습한다"**는 뜻입니다. 즉 멀티스텝/원스텝은 **학습 방식이 아니라 추론 방식**의 구분입니다.
...
**원스텝을 처음부터 학습시키면 왜 안 될까요?** 목표 분포가 복잡할 때 "한 방에 도달하는 방향"이라는 게 애매하기 때문입니다. 같은 노이즈에서 갈 수 있는 그럴듯한 결과가 여러 개인데, 한 방향만 내놓으라고 하면 모델은 **그 여러 정답의 평균**을 뱉습니다. 결과는 흐릿한 이미지죠.

**그래서 2단계로 갑니다.**

```text
Stage I  : 멀티스텝으로 제대로 배운다      <- "천천히 여러 번 나눠 걸으며 길을 익힌다"
Stage II : 그 지식을 원스텝으로 압축한다   <- "익힌 길을 지름길 하나로 요약한다"
```
...
Stage I의 학습 타깃은 x0-prediction(원본 예측) 형식입니다.

```text
y_t = (1-t)*y_0 + t*eps                              ... (식 3)
   (즉 깨끗한 목표 y_0 와 무작위 노이즈 eps 를 t 비율로 섞은 값)

L_FM = E[ || X_theta(y_t, t | c) - y_0 ||^2 ]        ... (식 4)
   (네트워크가 섞인 값에서 원본 y_0 를 직접 맞히도록 학습)

v_theta(y_t, t | c) = ( y_t - X_theta(y_t, t | c) ) / t   ... (식 5)
   (x0-prediction 을 velocity(속도) 형태로 변환하는 공식)
```
...
| 단계 | 해상도 규칙 | 데이터 | 목적 |
|---|---|---|---|
| Pre-train 1 | 256×256 고정 | 1.2B | 낮은 비용으로 넓은 시각-언어 정렬 |
| Pre-train 2 | 512픽셀 native 비율 (원본 종횡비 유지, 픽셀 예산 512² 수준) | 600M | 고품질 부분집합으로 이동 |
| Pre-train 3 | 1024픽셀 native 비율 | 300M | 세부 묘사, 레이아웃 충실도, 텍스트 렌더링, 미학 |
| **SFT** | 1024픽셀 native 비율 | 150M | 더 엄격한 필터 + 능력 표적 데이터 가중 → **Mage-Flow-Base** |
