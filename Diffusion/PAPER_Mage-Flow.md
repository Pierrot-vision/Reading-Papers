...
#### 7.2.1 ⭐ 다중 참조(multi-reference)는 3D RoPE로 충분한가

*왜 이걸 따로 보는가: README와 코드가 참조 이미지 여러 장을 받도록 열어뒀는데, frame 축에 배정된 차원이 16개(전체 128 중 12.5%)뿐입니다. 이게 병목인지 확인이 필요합니다.*

**결론부터 — 용량은 문제가 아니고, 다른 세 가지가 문제입니다.**

**(1) 용량 검증 — 충분합니다**

frame 축 16차원 = 주파수 쌍 8개. `theta=10000` 기준으로 직접 계산하면:

```text
주파수: [1.0, 0.316, 0.1, 0.0316, 0.01, 0.00316, 0.001, 0.000316]

프레임 인덱스별 최고주파수 회전각
  idx=0 :   0.0도
  idx=1 :  57.3도     <- 인접 이미지 간 57 도씩 분리
  idx=2 : 114.6도
  idx=3 : 171.9도
```

| 프레임 간격 | 코사인 유사도 (1.0 = 구분 안 됨) |
|---|---|
| 0 (같은 이미지) | 1.0000 |
| 1 | 0.9356 |
| 2 | 0.7960 |
...
**참조 1~3장을 구분하기엔 충분하고도 남습니다.** `pos_index = torch.arange(4096)`이라 인덱스 상한도 넉넉합니다.

> 다만 8개 주파수 중 실질적으로 일하는 건 **상위 2~3개뿐**입니다. 나머지(0.001, 0.0003…)는 인덱스가 1~3 변해도 거의 회전하지 않습니다. **실효 차원은 4~6dim** 정도로 보는 게 정확합니다.

**(2) ⭐ 진짜 문제 1 — 모든 참조가 h/w 좌표를 공유합니다**

편집 파이프라인은 **모든 참조를 타깃 해상도로 리사이즈**합니다.
...
그리고 RoPE에서 h/w 주파수는 `idx`와 무관합니다.

```python
def _compute_video_freqs(self, frame, height, width, idx=0):
    freqs_frame  = freqs_pos[0][idx : idx + frame]   # <- idx 의존
    freqs_height = ...(height)...                    # <- idx 무관
    freqs_width  = ...(width)...                     # <- idx 무관
```

**결과:**

```text
참조1 토큰:  [frame=1: 16dim] + [h=5: 56dim] + [w=7: 56dim]
참조2 토큰:  [frame=2: 16dim] + [h=5: 56dim] + [w=7: 56dim]
              ↑ 여기만 다름       ↑ 동일          ↑ 동일
```

"이 토큰이 어느 이미지 소속인가"라는 정보가 **위치 신호의 12.5%(실효 3~5%)에만 실려 있습니다.**

**다른 모델들과 비교하면 약한 설계입니다.**

| 모델 | 참조 구분 방식 | 할당 비중 |
|---|---|---|
| **Mage-Flow** | frame 축 (16/128) | **12.5%** |
| FLUX.2-Klein-4B | 4축 균등 중 인덱스 축 (32/128) | **25%** |
| [[paper_any2anytryon]] | **같은 2D 평면의 다른 좌표로 오프셋** | h/w 자체가 달라짐 |
| [[paper_query_kontext]] | shifted RoPE | 동일 계열 |

Any2AnyTryon 방식은 참조를 **공간적으로 분리**하므로 신호가 훨씬 강합니다. Mage-Flow는 오프셋 없이 frame 축에만 의존합니다.

**(3) 진짜 문제 2 — 상대 비중이 낮고, 증류 단계에서 빠졌습니다 (지배적 요인)**

| 단계 | Multi Reference 비중 | **절대 장수** |
|---|---|---|
| 편집 사전학습 (45M) | 2.1% | **약 95만 장** (§9.0.8) |
| Edit Stage 1·2 혼합 | **≤0.5%** (논문 명시) | Stage 1 기준 최대 17.5만 장 |
| **증류셋 (250K)** | ⭐ **범주 자체가 없음** | **0장** (§9.0.9) |

> 📌 **오해하기 쉬운 지점**: "학습을 안 했다"가 아닙니다. **95만 장은 절대량으로 결코 적지 않습니다.** 문제는 ① 편집 데이터 전체에서의 **상대 비중이 0.5~2.1%로 낮고** ② **증류 단계에서 완전히 빠졌다**는 점입니다.

**그래서 변형별로 예상이 갈립니다.**

| 변형 | 다중 참조 노출 | 예상 |
|---|---|---|
| Edit-Base / Edit (30스텝) | 95만 장 학습 | ⚠️ 어느 정도 작동할 것 |
| **Edit-Turbo (4스텝)** | 증류셋에 **0장** | ❌ **취약할 것** |

Turbo 증류는 teacher의 행동을 student로 옮기는 과정인데, **그 데이터에 다중 참조가 없으면 teacher가 갖고 있던 능력이 전이되지 않습니다.** 다른 지표에서는 Turbo가 다 이기지만(§11.3), 다중 참조만은 **30스텝 버전이 나을 가능성**이 큽니다. TextEdit에서 이미 같은 패턴이 확인됐습니다(Turbo 12.77 vs 30스텝 14.14).

**(4) 진짜 문제 3 — 배치 의존성이 여기서 증폭됩니다**

§12.4의 문제가 다중 참조에서 더 나빠집니다.
...
`idx`가 **팩 전체에서의 위치**라, 참조가 많을수록 뒤 샘플이 밀립니다.

| 팩 구성 | 샘플2의 타깃 frame idx |
|---|---|
| 샘플1이 참조 1장 | **2** |
| 샘플1이 참조 3장 | **4** |

**참조 장수에 따라 뒤 샘플의 좌표가 달라집니다.** 배치 크기 1로 고정하는 게 안전합니다.

**(5) ⭐⭐ 가장 큰 문제 — 측정을 안 했습니다**

논문이 쓴 벤치마크 전부를 보면:

| 벤치마크 | 다중 참조를 재나? |
|---|---|
| ImgEdit / GEdit / TextEdit | ❌ 단일 소스 편집 |
| GenEval / DPG / TIIF / CVTG / LongText / OneIG | ❌ 생성 |

**다중 참조 능력을 재는 표준 벤치마크(OmniContext)가 있는데 측정하지 않았습니다.** [[paper_unicustom]]과 [[paper_uniref_image_edit]]은 둘 다 이걸로 평가하고, 그게 그 논문들의 핵심 성적표입니다.

즉 **README와 코드가 기능을 열어뒀지만, 그 성능에 대한 근거가 논문 어디에도 없습니다.** 코드 docstring도 "trained with up to 3, **but more are accepted**"라고만 씁니다 — 학습 범위와 허용 범위를 구분해 적은 조심스러운 표현입니다.

**정리**

| 항목 | 판정 |
|---|---|
| 16dim 3D RoPE가 용량 부족인가 | ❌ **아니요.** 참조 3장 구분엔 충분 (57도 분리) |
| ① 좌표 공유 | ⚠️ 모든 참조가 h/w 동일 → 구분이 frame 축 12.5%에만 의존 |
| ② 학습 데이터 | ❌ 사전학습 2.1%, 혼합 ≤0.5%, **증류셋 0%** ← **지배적** |
| ③ 배치 의존성 | ⚠️ 참조 장수가 뒤 샘플 좌표를 밀어냄 |
| ④ **검증 부재** | ❌ **OmniContext 등 다중 참조 벤치마크 미측정** |

**한 줄로**: 3D RoPE 자체는 다중 참조에 충분하고 95만 장을 학습하기도 했지만, **상대 비중이 0.5~2.1%로 낮고 증류셋에서 완전히 빠졌으며 측정된 적이 없습니다.** 30스텝 변형은 시도해볼 만하고, Turbo는 기대하기 어렵습니다.
...
| 텍스트 회전 | 적용 | 미적용 |
| **이미지 인덱스 축 비중** | **25%** (32/128) | **12.5%** (16/128) |

> ⚠️ 마지막 행이 **다중 참조 편집에서 불리하게 작용합니다.** Mage-Flow는 모든 참조를 타깃 해상도로 리사이즈하므로 h/w 좌표가 전부 동일해지고, "어느 이미지 소속인가"를 frame 축 16차원만으로 구분해야 합니다. Klein은 그 두 배를 씁니다. 용량 계산과 실제 영향은 **§7.2.1** 참조.
...
### Q23. 다중 참조 편집에서 3D RoPE(frame 16dim)는 문제없나요?

**용량은 문제없습니다. 다른 게 문제입니다.**

frame 축 16차원 = 주파수 쌍 8개인데, 인접 이미지 간 최고 주파수에서 **57.3도씩 벌어집니다.** 참조 3장 구분엔 충분합니다(간격 1의 코사인 유사도 0.9356, 간격 3은 0.6929).

진짜 문제는 넷입니다.

| 문제 | 내용 |
|---|---|
| ① **좌표 공유** | 모든 참조를 타깃 해상도로 리사이즈 → **h/w 112차원이 전부 동일**. 구분이 frame 16dim(12.5%)에만 의존. Klein은 25%, [[paper_any2anytryon]]은 아예 2D 오프셋으로 공간 분리 |
| ② **데이터** | 사전학습 2.1%(**약 95만 장 — 절대량은 적지 않음**), 혼합 ≤0.5%, **증류셋 0장** ← 지배적 요인 |
| ③ **배치 의존성** | 참조 장수에 따라 뒤 샘플의 frame idx가 밀림(§12.4) |
| ④ **검증 부재** | **OmniContext 등 다중 참조 벤치마크를 아예 측정 안 함** |

②가 결정적인데, **"학습을 안 했다"가 아니라 "상대 비중이 낮고 증류에서 빠졌다"** 는 뜻입니다. 그래서 **30스텝 Edit/Edit-Base는 어느 정도 되고, 4스텝 Turbo가 특히 취약**할 것으로 보입니다. 자세히는 **§7.2.1** 참조.

### Q24. Qwen-Image와 학습 환경이 같은 건가요?
...
9. **다중 참조 편집은 미검증 기능**(§7.2.1) — 사전학습에서 약 95만 장을 보긴 했지만 편집 데이터 내 비중이 **0.5~2.1%**에 그치고, **증류셋에는 범주 자체가 없어 Turbo로 전이되지 않았으며**, 표준 벤치마크(OmniContext)로 **측정한 적이 없습니다.** 게다가 모든 참조를 같은 해상도로 리사이즈해 h/w 좌표가 동일해지므로, 이미지 구분이 frame 축 12.5%에만 의존합니다(Klein은 25%)
10. **강제 워터마크와 강제 콘텐츠 필터가 논문에 전혀 언급되지 않음.** 특히 워터마크는 초기 노이즈의 부호를 고정시키는 설계인데 어디에도 설명이 없고, 벤치마크가 이 조건에서 측정됐는지도 불명확. **OneIG Diversity가 26개 모델 중 최하위**(0.105)인 것과 연결될 가능성(§13.16-f)
11. **"one-step diffusion VAE"는 추론 시 확산이 아님** — 명명이 과함
12. 논문 Figure 5는 텍스트 stream에도 RoPE를 그려놨지만 공개 코드는 비활성화 — 그림과 코드 불일치
13. **논문이 밝히지 않은 혈통** — Transformer의 직계 조상은 SD3가 아니라 Qwen-Image(§13.3)
