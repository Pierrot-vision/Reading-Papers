...
### Q4. SemVAE 에 대해 자세히 설명해줘 역할 의미등등
...
### Q5. SeFi-Image 에서 사용하는 SemVAE는 여기와 동일해?
...
### Q6. SeFi-Image에서 사용되는 SemVAE는 공개되었어? / SFD 사용하는 SemVAE 는?
...
### Q8. 추론시에는 SemVAE가 필요없어?

> 왜 이 절이 있나: SemVAE를 "이미지를 넣어야 돌아가는 부품"으로 오해하기 쉬운데, 실제로는 생성 경로에 아예 등장하지 않는다. 이 사실이 SeFi가 SemVAE 없이 추론 코드를 배포할 수 있었던 이유다.

**필요 없다.** 인코더도 디코더도 쓰이지 않는다. 코드로 확정된다 (`inference.py` 실체와 채널 순서 함정은 → **알고리즘 4절 참조**):

```yaml
model:
  in_chans: 48
  semantic_chans: 16          # 48 = 32 텍스처 + 16 의미
  semfirst_delta_t: 0.3
vae:
  model_name: 'sdvae_f16d32'  # ← SemVAE 없음. SD-VAE 하나만 선언
```

**왜 필요 없나**: 의미 latent를 이미지에서 인코딩하는 게 아니라, **순수 노이즈에서 DiT가 직접 생성**하기 때문. 다 만들고 나면 그냥 버리고 텍스처 latent만 SD-VAE로 디코딩한다. 그러니 SemVAE 인코더가 낄 자리가 없다. (이게 "문제 아닌가?"라는 자연스러운 의심은 → **Q9 참조**)

**SemVAE 디코더는 더 극단적**이다 — 생성 경로엔 **영원히 등장하지 않는다.** 오직 SemVAE 자기 자신을 훈련할 때 복원 손실을 계산하려고만 쓰인다.

**그럼 SemVAE는 언제 필요한가:**

| 상황 | SemVAE 필요? |
|---|---|
| 노이즈에서 이미지 생성 (T2I, class-conditional) | ❌ 불필요 |
| **SFD 모델 학습/파인튜닝** | ✅ **필수** — 정답 의미 latent를 만들어야 함 (`extract_dinov2_feature.py` → SemVAE 인코더 → 미리 뽑아둔 latent 데이터셋) |
| 실제 이미지를 인코딩해야 하는 작업 (img2img, 편집, 인페인팅) | ✅ 인코더 필요 (단 두 논문 모두 미구현·미평가) |

SeFi가 추론 레포에 SemVAE를 안 넣고도 정상 동작하는 이유, 그러면서 SemVAE 가중치는 따로 HF에 올린 이유가 이걸로 설명된다 — **"추론엔 불필요, 학습엔 필수"**이기 때문. (공개 현황은 Q6 참조)

### Q9. "의미 latent를 이미지에서 인코딩하는 게 아니라, 순수 노이즈에서 DiT가 직접 생성하기 때문입니다." ?? 왜 그럼 문제 있는거 아냐?
...
**더 날카롭게 말하면**: 진짜 기여는 "의미를 먼저 보내라"가 아니라 **"의미를 *조금만* 먼저 보내라 — 완전히 앞세우면(Δt=1.0) exposure bias로 망한다"**는 발견이다. Δt는 *의미 가이드의 선명함* ↔ *학습-추론 불일치* 를 맞바꾸는 다이얼이고, Figure 5의 U자 곡선이 그 트레이드오프의 지도다. (Q9 참조)
