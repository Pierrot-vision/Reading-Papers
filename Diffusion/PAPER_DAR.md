...
### Q4. 참고 구현 코드에서 residual 부분이 어디야?

**그 코드에 residual은 없다 — 없는 게 DAR의 정체다.** "residual 부분이 어디 갔지?"라는 의문이 들었다면 코드를 제대로 읽은 것.
...
def forward(self, x, t_emb):
    x = x + self.attn(self.norm1(x))   # ← 이 "x +" 가 residual
    x = x + self.mlp(self.norm2(x))    # ← 여기도
    return x

# DARBlock
def forward(self, history, t_emb):
    h = self.route_attn(history, t_emb)        # ← residual이 있던 자리 (1)
    history.append(self.attn(self.norm1(h)))   # ← residual이 있던 자리 (2)
    h = self.route_mlp(history, t_emb)
    history.append(self.mlp(self.norm2(h)))
    return history
```

표준 블록의 `x = x + attn(...)` 한 줄이 하던 일이 DAR에서는 두 조각으로 쪼개져 대체됐다:

1. **"x + " (과거를 가져오는 부분)** → `route_attn(history, t_emb)`가 대신한다. 다만 "직전 상태에 무조건 더하기"가 아니라 "과거 출력 전부를 후보로 놓고 softmax 비율로 골라 담기"로 바뀐 것.
2. **"x = " (결과를 다음으로 넘기는 부분)** → `history.append(...)`가 대신한다. 누적된 합 x를 갱신하는 게 아니라, 이번 sublayer의 출력을 역사 목록에 등록만 해두고, 필요할 때 뒷층이 알아서 꺼내 쓰게 한다.

그래서 표준 블록에는 "누적 합"이라는 상태 변수 x가 흐르는데, DARBlock에는 그게 아예 없고 **history라는 목록**만 흐른다. 이게 논문이 "non-incremental"(비증분)이라 부르는 이유.

**residual의 역할은 누가 하나** — residual이 사라졌는데 그 순기능(앞쪽 정보가 뒤까지 살아서 도달하게 하기, gradient 지름길 만들기)은, history 안에 첫 입력 `x_embed`($v_0$)부터 모든 과거 출력이 살아있고 α가 언제든 거기에 가중치를 줄 수 있다는 점이 이어받는다. 수학적으로 보면 **표준 residual은 "모든 과거에 계수 1을 주는 고정 배합"이라는 특수 경우**이고, DAR은 그 배합을 학습 가능하게 일반화한 것. 다만 DAR의 가중치는 softmax라 합이 1로 정규화되니, 계수 1로 무한정 쌓여 크기가 100배 팽창하던 문제까지 같이 사라진다.

**한 줄**: residual은 삭제됐고, 그 자리(⊕)를 `route_*` 호출이 차지했으며, residual의 순기능은 history + α가 이어받았다 — Figure 4 왼쪽 그림에서 ⊕ 대신 분홍 α 노드가 박혀 있는 게 정확히 이 코드다.

### Q5. history가 x야?

아니다 — 그런데 아주 가까운 관계다. **history는 "재료 목록"이고, 표준 블록의 x는 "그 재료를 다 부어 섞은 국물"이다.**

내용물을 구체적으로 보면:

```python
history = [
    x_embed,      # v0: 패치 임베딩 직후의 첫 입력
    attn1_출력,    # v1: 블록1 attention의 출력 (그것만, 합 아님)
    mlp1_출력,     # v2: 블록1 MLP의 출력
    attn2_출력,    # v3: ...
    ...
]                 # 각 원소가 (B, N, d) 텐서로 따로따로 보관됨
```

표준 DiT의 x와의 수학적 관계:

- **표준 블록의 x** = history 원소를 **전부 계수 1로 더한 합** (x = v0 + v1 + v2 + ...). 더한 순간 개별 재료는 구분 불가능하게 섞여버리고, 이 "누적 합" 하나만 다음 층으로 흐른다.
- **DAR의 history** = 더하지 않고 재료를 **따로따로 들고 다니는 것**. 그래서 각 층의 α가 "v1은 45%, v3은 30%..." 하고 개별적으로 가중치를 줄 수 있다. 한번 합쳐버리면 다시 풀 수 없으니, 골라 담기를 하려면 반드시 따로 보관해야 한다.

이게 DAR의 메모리 비용이 커지는 이유이기도 하다 — 표준 residual은 텐서 하나(x)만 들고 다니면 되는데, DAR은 층 수만큼의 텐서 목록을 들고 다녀야 해서 청크 압축(→ 알고리즘 3절)이 필요해진 것.

참고로 코드에서 x와 **역할이** 가장 비슷한 변수는 history가 아니라 `h`다 — "이번 sublayer에 들어가는 입력"이라는 자리가 같기 때문. 차이는, 표준 x는 층을 지나며 계속 이어지는 누적 상태인 반면, DAR의 h는 매 층마다 history에서 새로 조립해 만들고 쓰고 버리는 일회용이라는 점.

**한 줄**: x = 다 섞인 누적 합(텐서 1개), history = 섞기 전 재료 목록(텐서 l개), h = 매번 새로 조립한 이번 층 입력. 표준 residual은 "history를 항상 계수 1로 합쳐 x로 만든 것"이라는 특수 경우다.

### Q6. 이전 레이어의 x를 저장해서 그때의 softmax로 골라 담는 거고, 현재 x에 추가하는 건 아닌 거지?

거의 정확 — 두 문장 다 핵심을 잡았고, 첫 문장에 미세 교정 하나만 하면 완벽하다.

#### 교정 하나: 저장하는 건 "이전 레이어의 x"가 아니라 "이전 레이어의 출력물(v)"

미묘하지만 중요한 구분:

- x(조립된 입력, 코드의 h)는 **저장 안 한다** — 매 층에서 조립해서 쓰고 바로 버리는 일회용 (→ Q5)
- history에 저장되는 건 각 sublayer가 **만들어낸 출력물** v (attn 출력, mlp 출력 각각) — 섞이기 전의 순수한 재료

이 구분이 중요한 이유: x는 이미 "여러 재료가 섞인 결과물"이라 그걸 저장하면 골라 담기의 의미가 없어진다. 섞기 전 원재료(v)를 따로 보관해야 각 층이 "층 3의 attn 출력만 45%" 같은 세밀한 선택을 할 수 있다.

#### "그때의 softmax로 골라 담기" — 정확하다

"그때"가 이중으로 맞다: **층마다**(층 10과 층 20이 각자 자기 query로 새로 softmax를 계산) + **timestep마다**(같은 층이라도 t가 다르면 배합이 다름). 매번 그 자리에서 새로 계산하는 즉석 배합.

#### "현재 x에 추가해야 하는 건 아니네?" — 정확히 그게 없어진 것

표준 residual에는 강제 조항이 있었다: "네 출력은 무조건 지금까지의 합에 +1 계수로 추가된다." DAR은 이 강제 추가가 사라지고, 이번 층의 출력도 **특별대우 없이 history에 등록만** 된다. 다음 층이 그걸 많이 쓸지(α 크게) 거의 무시할지(α 작게)는 전적으로 학습된 라우터가 결정한다. 직전 층 출력이라고 해서 자동 우선권이 없다 — 후보 줄에 똑같이 선다.

이게 논문 용어 **"non-incremental"(비증분)의 정확한 의미**: 증분(현재 상태 + 추가분)이라는 개념 자체가 없고, 매 층이 역사 전체에서 처음부터 다시 조립.

#### 예외적 디테일: Dynamic 변형에서 직전 출력의 특별한 역할

Dynamic 변형에서는 직전 출력이 약간 특별한 역할을 하나 갖는다. **선택되는 후보로서**가 아니라 **query를 만드는 재료로** 쓰이는 것 (q = W × 직전 출력, → 알고리즘 2절). "방금 만들어진 걸 보고 다음에 뭘 가져올지 정한다"는 구조라, 직전 출력이 '장바구니에 자동으로 담기는 것'이 아니라 '쇼핑 목록을 쓰는 손'이 되는 셈.

**정리**: 저장 = 각 층의 출력물 v (조립된 x 아님), 조립 = 층마다·t마다 즉석 softmax, 그리고 "현재에 무조건 더하기"는 폐지 — 직전 출력도 후보 중 하나일 뿐.

### Q7. 이렇게 하면 뭐가 이익이야?

한 마디로: **같은 품질을 8.75배 적은 학습으로 얻는다** — 이익의 본질은 "더 좋은 그림"보다 "훨씬 싼 학습"이다. 진단했던 병 세 개(→ 알고리즘 1절)가 어떻게 낫는지로 풀면:

#### 이익 1. 신호 크기 팽창이 구조적으로 사라진다

표준 residual은 계수 1로 무한정 더하니 신호 크기가 층을 지나며 100배까지 부풀었다. 국물이 너무 진해져서 뒤쪽 층이 새로 넣는 재료는 맛이 안 나는 상태(희석) — 즉 **깊은 층이 일해도 티가 안 나는** 구조. DAR은 softmax라 가중치 합이 항상 1이니, 아무리 깊어도 크기가 일정하고 뒤쪽 층의 기여가 묻히지 않는다. 별도 트릭 없이 수식 형태 자체가 이 병을 치료한다.

#### 이익 2. gradient가 모든 층에 직행한다

표준 구조에서는 손실의 gradient가 뒤쪽 블록에서 거의 0으로 죽어서, 깊은 층들이 사실상 학습 신호를 못 받았다. DAR에서는 마지막 층이 history를 통해 **모든 과거 층과 직접 연결**되어 있으니, gradient가 층층이 거쳐 내려갈 필요 없이 α 경로로 초기 층까지 바로 간다. 모든 층이 학습 신호를 제대로 받는 것 — 이게 수렴이 빨라지는 주 엔진.

#### 이익 3. 층들의 역할이 분화된다

표준 구조에서는 인접 블록 출력이 코사인 유사도 0.9 이상 — 이웃끼리 거의 같은 일을 하며 파라미터를 낭비했다. DAR에서는 각 층이 "필요한 것만 골라 받는" 구조라, 층마다 다른 조합을 받아 다른 일을 하도록 압력이 생긴다. 같은 675M 파라미터에서 실효 용량이 커지는 셈. (증거: 파라미터를 더 늘린 SiT-Plus 752M보다 DAR 675M이 더 좋음 — 크기가 아니라 활용의 문제였다는 것.)

#### 이익 4. diffusion 전용 보너스 — 단계마다 배선이 바뀐다

노이즈 많은 단계에선 구도(전역 의미)가, 마무리 단계에선 질감(고주파)이 필요한데, 표준 residual은 어느 단계든 같은 고정 배선이다. DAR은 t를 보고 배합을 바꾸니 **한 모델이 timestep마다 사실상 다른 배선의 네트워크처럼** 동작한다. t를 빼면 성능이 무너지는 ablation(FID 7.97 → 11.51, Table 2)이 이 보너스가 진짜임을 보여준다. DMD 4-step 증류에서 고주파 디테일이 덜 뭉개지는 것도 같은 맥락.

#### 이 넷이 합쳐진 실전 결과

- FID 9.67 → 7.56을 **3분의 1 스텝**으로
- baseline 수렴 품질까지 **8.75배 적은 스텝** = GPU 시간·전기료가 그만큼 절약
- REPA 같은 기존 가속 기법 위에 얹어도 초반 2배 추가 가속 (겹치지 않는 이익)

#### 대가는?

재료를 따로 보관하니 메모리가 늘고 계산이 추가되는데, 청크 압축(활성화 85% 절감)과 Triton 커널(11.5배 가속)로 상쇄 (→ 알고리즘 3·4절). 그리고 솔직한 한계 하나 — CFG를 쓴 최종 품질에서는 이득이 얼마 안 남는다(2.15 vs 2.05).

**한 줄**: "최종 도달점을 크게 올리는 기술이라기보다, 그 도달점까지 가는 기찻삯을 8분의 1로 줄이는 기술" — 학습 비용이 수십만 GPU-hour 단위인 요즘 T2I 판에서는 이게 곧 돈이다.
