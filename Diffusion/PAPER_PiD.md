...
### Q13. 제목의 "Fast"는 무슨 의미로 읽어야 하나?

**절대적으로 빠르다가 아니라, 조건부·상대적 의미다.** PiD는 모든 디코더보다 빠르지 않다.

1. **무엇보다 빠른가 — VAE가 아니라 "확산(diffusion) SR cascade" 대비.** 확산 기반 1-step SR(SeedVR2·TSD-SR·InvSR, 720~1270 ms) 대비 **3~6배 빠름**(~210 ms, §13.1). 반면 Real-ESRGAN 같은 단순 upsampler는 62 ms로 **PiD보다 빠르다**(대신 품질 낮음). 즉 "fast"는 *고품질 생성형 디코딩 중에서* 빠르다는 뜻.
2. **어디서 빠른가 — 고해상도(megapixel)에서.** 저해상도(256px)는 VAE가 더 싸다. 하지만 4K로 가면 VAE는 OOM(80GB도 터짐), PiD는 22.5GB·실용 속도(§13.3). 논문이 지적한 "decoder becomes costly at megapixel scale"을 푸는 게 fast.
3. **확산인데도 빠르다 (역설 해소).** 보통 diffusion은 느린데, ① 4-step distillation ② decode+upsample 통합(cascade 1패스) ③ early termination(LDM 후반 step 생략) ④ PixDiT 압축 구조 덕에 비확산 급 속도에 근접.

**제목의 진짜 메시지**: "Fast **and** High-Resolution"은 보통 상충하는 둘(고해상도=느림)을 **동시에** 잡았다는 선언이다. "fast"를 단독으로 읽지 말고 **"고해상도인데도 빠르다"** 로 한 쌍으로 읽어야 한다.

| 잘못된 해석 | 올바른 해석 |
|---|---|
| 모든 디코더보다 빠르다 | ❌ VAE+ESRGAN이 더 빠름 |
| 절대적으로 빠르다 | ❌ 고해상도·고품질 한정 |
| 확산 SR cascade보다 3~6배 | ✅ 주된 비교 |
| 메가픽셀에서 VAE OOM 구간을 1초 미만으로 | ✅ |
