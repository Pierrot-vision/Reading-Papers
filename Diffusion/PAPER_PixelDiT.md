...
**압축이 얼마나 줄이나 (수치)** — 시퀀스 길이는 p²배, 어텐션 비용은 그 제곱인 **p⁴배** 준다(p=16이면 16의 네제곱 = **65,536배**).

1024×1024 이미지로 실감하면:

| | 픽셀 그대로 | 16×16 압축 후 |
|---|---|---|
| 어텐션 토큰 수 | 1,048,576개 (백만) | 4,096개 |
| 시퀀스 길이 | — | **256배 감소** (p²=256) |
| 어텐션 연산량 | — | **약 65,536배 감소** (p⁴) |

→ 백만 토큰끼리 어텐션은 물리적으로 불가능하지만, 압축하면 4천 토큰 수준이 되어 감당 가능. "픽셀 직접 확산은 비싸서 안 된다"는 통념을 깬 결정타.

전체 연산량으로 봐도(§실험 참조) PixelDiT 311 GFLOPs는 같은 품질의 JiT-G(766)보다 **약 2.5배**, PixelFlow-XL(5,818)보다 **약 18.7배** 적다. 파라미터도 797M vs JiT-G 2B로 **약 2.5배** 작다.
